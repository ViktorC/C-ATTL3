...
class NesterovMomentumSGDOptimizer : public MomentumSGDOptimizer<Scalar, Rank, Sequential> {
 ...
 public:
   ...
   * and less than 1.
   */
  inline NesterovMomentumSGDOptimizer(LossSharedPtr<Scalar, Rank, Sequential> loss, std::size_t batch_size = 1,
                                      Scalar init_learning_rate = 1e-3, Scalar annealing_rate = 1e-3,
                                      Scalar momentum = .9)
      : Base::MomentumSGDOptimizer(loss, batch_size, init_learning_rate, annealing_rate, momentum){};
 ...
 protected:
  inline void _update_params(const std::vector<Parameters<Scalar>*>& params_vec, std::size_t epoch,
    ...
    for (auto params_ptr : params_vec) {
      ...
      Base::params_grad_vec[i] = old_acc_params_grad * Base::momentum - params_ptr->get_grad() * learning_rate;
