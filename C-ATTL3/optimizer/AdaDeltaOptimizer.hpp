...
class AdaDeltaOptimizer : public SGDOptimizer<Scalar, Rank, Sequential> {
 ...
 public:
   ...
   * @param decay The decay rate of the accelerated accumulated parameter
  ...
  inline AdaDeltaOptimizer(LossSharedPtr<Scalar, Rank, Sequential> loss, std::size_t batch_size = 1,
      ...
      : SGDOptimizer<Scalar, Rank, Sequential>::SGDOptimizer(loss, batch_size), decay(decay), epsilon(epsilon) {
 ...
 protected:
  ...
  /**
   * A struct containing the moving averages of the squared gradients and
   * squared updates of a layer.
   */
