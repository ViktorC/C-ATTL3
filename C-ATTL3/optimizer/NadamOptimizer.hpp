...
/**
 * A class template for the Nesterov accelerated Adam (Nadam) optimization
 * algorithm.
 *
  ...
  inline void _update_params(const std::vector<Parameters<Scalar>*>& params_vec, std::size_t epoch,
    ...
    for (auto params_ptr : params_vec) {
      ...
      params_ptr->set_values(params_ptr->get_values() -
                             ((params_grad * (Base::l1_decay * l1_corr) +
                               grad_norms.params_grad_l1 * ((1 - Base::l1_decay) * l1_next_corr))
                                  .array() *
                              Base::learning_rate /
                              ((grad_norms.params_grad_l2 * l2_corr).array() + Base::epsilon).sqrt())
                                 .matrix());
    }
  }
};
