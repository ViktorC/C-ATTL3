...
class AMSGradOptimizer : public AdamOptimizer<Scalar, Rank, Sequential> {
 ...
 public:
   ...
   * expected to be greater than 0.
   * @param l1_decay The decay rate of the accumulated parameter gradients. It
   * is expected to be in the range [0,1]. The greater it is, the faster the
   * accumulated gradients decay.
   ...
   * faster the accumulated squared gradients decay.
 ...
 protected:
  inline void _fit(const std::vector<Parameters<Scalar>*>& params_vec) {
    ...
    for (std::size_t i = 0; i < params_grad_l2_max.size(); ++i) params_grad_l2_max[i] = Base::pgn_vec[i].params_grad_l2;
  }
  inline void _update_params(const std::vector<Parameters<Scalar>*>& params_vec, std::size_t epoch,
    ...
    for (auto params_ptr : params_vec) {
      ...
      grad_norms.params_grad_l2 =
          grad_norms.params_grad_l2 * (1 - Base::l2_decay) + params_grad.cwiseProduct(params_grad) * Base::l2_decay;
      ...
      params_ptr->set_values(params_ptr->get_values() - (grad_norms.params_grad_l1.array() * Base::learning_rate /
                                                         (params_grad_l2_max.array() + Base::epsilon).sqrt())
                                                            .matrix());
    }
  }
