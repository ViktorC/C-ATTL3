 ...
 protected:
  inline void _fit(const std::vector<Parameters<Scalar>*>& params_vec) {
    ...
    for (auto params_ptr : params_vec) {
      if (!params_ptr->are_optimizable() || params_ptr->are_frozen()) continue;
      ParamsGradNorms pgn;
  ...
  inline void _update_params(const std::vector<Parameters<Scalar>*>& params_vec, std::size_t epoch,
    ...
    for (auto params_ptr : params_vec) {
      ...
      grad_norms.params_grad_l2 =
          grad_norms.params_grad_l2 * (1 - l2_decay) + params_grad.cwiseProduct(params_grad) * l2_decay;
      params_ptr->set_values(params_ptr->get_values() -
                             ((grad_norms.params_grad_l1 * (learning_rate * l1_corr)).array() /
                              ((grad_norms.params_grad_l2 * l2_corr).array() + epsilon).sqrt())
                                 .matrix());
    }
  }
  const Scalar learning_rate, l1_decay, l2_decay, epsilon;
  /**
   * A struct containing the moving averages of the first and second norms of
   * the parameter gradients of a layer.
   */
