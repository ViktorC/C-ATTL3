...
class SGDOptimizer : public Optimizer<Scalar, Rank, Sequential> {
 ...
 public:
  inline SGDOptimizer(LossSharedPtr<Scalar, Rank, Sequential> loss, std::size_t batch_size)
      : Base::Optimizer(loss), batch_size(batch_size) {
    assert(batch_size > 0);
  }
 ...
 protected:
  inline Scalar _train(typename Base::Net& net, typename Base::Provider& training_prov, std::size_t epoch,
    ...
    for (auto params_ptr : params_vec) {
      if (params_ptr->are_optimizable() || !params_ptr->are_frozen()) optimizable_params_vec.push_back(params_ptr);
    }
    ...
    while (training_prov.has_more()) {
      ...
      /* Divide the gradient by the batch size to decouple the learning rate and
       * the batch size hyper-parameters. Use the nominal batch size as the
       * denominator even if the actual batch size is different (in case the
       * number of samples in the data set is not divisible by the batch size
       * and the last batch of the epoch contains fewer instances than the
       * others) to make sure that the magnitude of the gradient is
    ...
    if (verbose) {
      std::cout << std::left << std::setw(20) << "\ttraining obj loss: " << std::right << std::to_string(mean_obj_loss)
                << std::endl;
      std::cout << std::left << std::setw(20) << "\ttraining reg loss: " << std::right << std::to_string(mean_reg_loss)
                << std::endl;
    }
  ...
  inline Scalar _test(typename Base::Net& net, typename Base::Provider& test_prov, std::size_t epoch, bool verbose) {
    ...
    if (verbose) {
      std::cout << std::left << std::setw(20) << "\ttest obj loss: " << std::right << std::to_string(mean_obj_loss)
                << std::endl;
      std::cout << std::left << std::setw(20) << "\ttest reg loss: " << std::right << std::to_string(reg_loss)
                << std::endl;
    }
   ...
   */
  virtual void _update_params(const std::vector<Parameters<Scalar>*>& params_vec, std::size_t epoch,
