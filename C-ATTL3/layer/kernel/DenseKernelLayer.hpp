	...
	inline DenseKernelLayer(const typename Root::Dims& input_dims, std::size_t output_size,
			...
			Scalar bias_grad_max_l2_norm = 0) :
				Base::KernelLayer(input_dims, output_size,
						std::make_shared<HostParameters<Scalar>>(input_dims.get_volume(), output_size, weight_init,
								true, weight_reg, weight_clip, weight_max_l1_norm, weight_max_l2_norm,
								weight_grad_clip, weight_grad_max_l1_norm, weight_grad_max_l2_norm),
						std::make_shared<HostParameters<Scalar>>(1, output_size,
								std::make_shared<ZeroParameterInitialization<Scalar>>(), true, bias_reg, bias_clip,
