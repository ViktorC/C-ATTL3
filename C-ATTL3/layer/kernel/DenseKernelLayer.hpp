...
class DenseKernelLayer : public KernelLayer<Scalar, Rank> {
 ...
 public:
  ...
  inline DenseKernelLayer(const typename Root::Dims& input_dims, std::size_t output_size,
      ...
      : Base(input_dims, {output_size},
             std::make_shared<StandardParameters<Scalar>>(
                 input_dims.get_volume(), output_size, true, weight_init, weight_reg, weight_clip, weight_max_l1_norm,
                 weight_max_l2_norm, weight_grad_clip, weight_grad_max_l1_norm, weight_grad_max_l2_norm),
             std::make_shared<StandardParameters<Scalar>>(
                 1, output_size, true, std::make_shared<ZeroParameterInitialization<Scalar>>(), bias_reg, bias_clip,
  ...
  inline Root* clone() const { return new DenseKernelLayer(*this); }
  inline Root* clone_with_shared_params() { return new DenseKernelLayer(*this, true); }
  ...
  inline typename Root::Data pass_forward(typename Root::Data in, bool training) {
    ...
    Matrix<Scalar> out_mat = (in_mat_cache * Base::weights->get_values()).rowwise() + Base::bias->get_values().row(0);
  ...
  inline typename Root::Data pass_back(typename Root::Data out_grad) {
    ...
    // Compute the gradient of the outputs with respect to the weights and the
    // bias.
