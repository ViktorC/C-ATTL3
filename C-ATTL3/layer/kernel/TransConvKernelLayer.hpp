...
class TransConvKernelLayerBase : public KernelLayer<Scalar, Rank> {
 ...
 protected:
  inline TransConvKernelLayerBase(const typename Root::Dims& input_dims, std::size_t filters,
                                  ...
                                  std::size_t horizontal_padding, std::size_t vertical_stride,
                                  std::size_t horizontal_stride, std::size_t vertical_dilation,
      ...
      : Base(input_dims,
             ...
             std::make_shared<StandardParameters<Scalar>>(
                 input_dims.template extend<3 - Rank>()(2), receptor_height * receptor_width * filters, true,
             ...
             std::make_shared<StandardParameters<Scalar>>(
                 ...
                 calculate_adjusted_output_dims(input_dims, filters, receptor_height, receptor_width, vertical_padding,
                     ...
                     .get_volume(),
                 true, std::make_shared<ZeroParameterInitialization<Scalar>>(), bias_reg, bias_clip, bias_max_l1_norm,
                 bias_max_l2_norm, bias_grad_clip, bias_grad_max_l1_norm, bias_grad_max_l2_norm)),
        filters(filters),
  ...
  inline Tensor<Scalar, 4> _pass_forward(Tensor<Scalar, 4> in, bool training) {
    ...
    /* Given the values of the stretched out receptor patches, accumulate them
    ...
    for (std::size_t i = 0; i <= padded_width - dil_receptor_width; i += horizontal_stride) {
      ...
      for (std::size_t j = 0; j <= padded_height - dil_receptor_height; j += vertical_stride) {
        ...
        TensorMap<Scalar, 4> out_patch(out_conv_mat_block.data(), rows, receptor_height, receptor_width,
                                       ext_output_dims(2));
  ...
  inline Tensor<Scalar, 4> _pass_back(Tensor<Scalar, 4> out_grad) {
    ...
    Base::bias->accumulate_grad(MatrixMap<Scalar>(out_grad.data(), rows, ext_output_dims.get_volume()).colwise().sum());
    // Spatial padding.
    if (vertical_padding > 0 || horizontal_padding > 0) out_grad = Tensor<Scalar, 4>(out_grad.pad(paddings));
    ...
    for (std::size_t i = 0; i <= padded_width - dil_receptor_width; i += horizontal_stride) {
      ...
      for (std::size_t j = 0; j <= padded_height - dil_receptor_height; j += vertical_stride) {
        ...
        out_grad_conv_mat.block(patch_ind, 0, rows, receptor_vol) = MatrixMap<Scalar>(patch.data(), rows, receptor_vol);
        patch_ind += rows;
      }
    }
    assert(patch_ind == total_patches);
 ...
 private:
  ...
  inline static Dimensions<std::size_t, Rank> calculate_adjusted_output_dims(
      const typename Root::Dims& input_dims, std::size_t filters, std::size_t receptor_height,
      std::size_t receptor_width, std::size_t vertical_padding, std::size_t horizontal_padding,
      std::size_t vertical_stride, std::size_t horizontal_stride, std::size_t vertical_dilation,
      std::size_t horizontal_dilation) {
  ...
  Array4 prev_out_conversion_dims, patch_offsets, patch_extents, dil_strides, no_padding_offsets, no_padding_extents;
...
/**
 * A class template for a transposed 2D convolutional layer operating on rank-3
 * data batches (rank-4 tensors). The results of the convolutions of the filters
 * and the input tensor are concatenated along the highest (4th) rank of the
 * output tensor.
 *
...
class TransConvKernelLayer : public TransConvKernelLayerBase<Scalar, Rank> {
 ...
 public:
   ...
   * @param vertical_stride The vertical transposed convolution stride i.e. the
   * number of elements by which the receptor is to be shifted along the height
   * of the input tensor.
   * @param horizontal_stride The horizonzal transposed convolution stride i.e.
   * the number of elements by which the receptor is to be shifted along the
  ...
  inline TransConvKernelLayer(const typename Root::Dims& input_dims, std::size_t filters,
                              ...
                              Scalar bias_grad_clip = 0, Scalar bias_grad_max_l1_norm = 0,
                              Scalar bias_grad_max_l2_norm = 0)
      : TransConvBase::TransConvKernelLayerBase(
            input_dims, filters, receptor_height, receptor_width, vertical_padding, horizontal_padding, vertical_stride,
            horizontal_stride, vertical_dilation, horizontal_dilation, weight_init, weight_reg, weight_clip,
            weight_max_l1_norm, weight_max_l2_norm, weight_grad_clip, weight_grad_max_l1_norm, weight_grad_max_l2_norm,
            bias_reg, bias_clip, bias_max_l1_norm, bias_max_l2_norm, bias_grad_clip, bias_grad_max_l1_norm,
            bias_grad_max_l2_norm) {}
  inline TransConvKernelLayer(const TransConvKernelLayer<Scalar, 3>& layer, bool share_params = false)
      : TransConvBase::TransConvKernelLayerBase(layer, share_params), batch_size(layer.batch_size) {}
  inline Root* clone() const { return new TransConvKernelLayer(*this); }
  inline Root* clone_with_shared_params() { return new TransConvKernelLayer(*this, true); }
...
/**
 * A class template for a transposed 2D convolutional layer operating on rank-2
 * data batches (rank-3 tensors). The results of the convolutions of the filters
 * and the input tensor are concatenated along the highest (3rd) rank of the
 * output tensor.
 *
 * \see https://arxiv.org/abs/1603.07285v1
 */
...
class TransConvKernelLayer<Scalar, 2> : public TransConvKernelLayerBase<Scalar, 2> {
 ...
 public:
  ...
  inline typename Root::Data pass_forward(typename Root::Data in, bool training) {
    ...
    assert(in.dimension(0) > 0);
    batch_size = in.dimension(0);
    return TransConvBase::_pass_forward(
               TensorMap<Scalar, 4>(in.data(), {batch_size, in.dimension(1), in.dimension(2), 1u}), training)
  ...
  inline typename Root::Data pass_back(typename Root::Data out_grad) {
    ...
    assert(out_grad.dimension(0) > 0 && batch_size == out_grad.dimension(0));
    Tensor<Scalar, 4> prev_out_grad = TransConvBase::_pass_back(TensorMap<Scalar, 4>(
        out_grad.data(), {batch_size, KernelBase::output_dims(0), KernelBase::output_dims(1) / TransConvBase::filters,
...
/**
 * A class template for a transposed 2D convolutional layer operating on rank-1
 * data batches (rank-2 tensors). The results of the convolutions of the filters
 * and the input tensor are concatenated along the highest (2nd) rank of the
 * output tensor.
 *
 * \see https://arxiv.org/abs/1603.07285v1
 */
...
class TransConvKernelLayer<Scalar, 1> : public TransConvKernelLayerBase<Scalar, 1> {
 ...
 public:
  ...
  TransConvKernelLayer(const typename Root::Dims& input_dims, std::size_t filters,
      ...
      : TransConvBase::TransConvKernelLayerBase(
            input_dims, filters, receptor_length, 1, padding, 0, stride, 1, dilation, 0, weight_init, weight_reg,
            weight_clip, weight_max_l1_norm, weight_max_l2_norm, weight_grad_clip, weight_grad_max_l1_norm,
            weight_grad_max_l2_norm, bias_reg, bias_clip, bias_max_l1_norm, bias_max_l2_norm, bias_grad_clip,
  ...
  inline typename Root::Data pass_back(typename Root::Data out_grad) {
    ...
    assert(out_grad.dimension(0) > 0 && batch_size == out_grad.dimension(0));
    Tensor<Scalar, 4> prev_out_grad = TransConvBase::_pass_back(TensorMap<Scalar, 4>(
        out_grad.data(), {batch_size, KernelBase::output_dims(0) / TransConvBase::filters, 1, TransConvBase::filters}));
    if (KernelBase::is_input_layer()) return typename Root::Data();
