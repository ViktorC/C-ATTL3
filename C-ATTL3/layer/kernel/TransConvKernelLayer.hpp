...
public:
	...
	inline Tensor<Scalar,4> _pass_forward(Tensor<Scalar,4> in, bool training) {
		...
		in_mat = MatrixMap<Scalar>(in.data(), total_patches, depth);
		Matrix<Scalar> out_conv_mat = in_mat * Base::weights->get_values();
		...
		Matrix<Scalar> out_mat(out.data(), rows, ext_output_dims.get_volume());
	...
	inline Tensor<Scalar,4> _pass_back(Tensor<Scalar,4> out_grad) {
		...
		Base::bias->update_grad(MatrixMap<Scalar>(out_grad.data(), rows,
		...
		assert(patch_ind == total_patches);
		Base::weights->update_grad(in_mat.transpose() * out_grad_conv_mat);
...
protected:
	inline TransConvKernelLayerBase(const Dimensions<std::size_t,Rank>& input_dims, std::size_t filters,
			...
			Scalar bias_grad_max_l1_norm = 0, Scalar bias_grad_max_l2_norm = 0) :
				Base::KernelLayer(input_dims, calculate_adjusted_output_dims(input_dims, filters,
						...
						std::make_shared<HostParameters<Scalar>>(input_dims.template extend<3 - Rank>()(2),
								receptor_height * receptor_width * filters, weight_init, true, weight_reg,
								weight_clip, weight_max_l1_norm, weight_max_l2_norm, weight_grad_clip,
								weight_grad_max_l1_norm, weight_grad_max_l2_norm),
						std::make_shared<HostParameters<Scalar>>(1, calculate_adjusted_output_dims(input_dims,
								filters, receptor_height, receptor_width, vertical_padding, horizontal_padding,
								vertical_stride, horizontal_stride, vertical_dilation, horizontal_dilation)
								.get_volume(), std::make_shared<ZeroParameterInitialization<Scalar>>(), true,
