...
class DropoutLayer : public Layer<Scalar, Rank> {
 ...
 public:
   ...
   * @param dropout_prob The probability of an element of the input tensor being
   * set to 0.
  ...
  inline DropoutLayer(const typename Base::Dims& dims, Scalar dropout_prob,
      ...
      : dims(dims), dropout_prob(dropout_prob), epsilon(epsilon), input_layer(false) {
    assert(dropout_prob > 0 && dropout_prob <= 1 && "dropout probability must be greater than 0 and no greater than 1");
    assert(epsilon > 0 && "epsilon must be greater than 0");
  }
  inline Base* clone() const { return new DropoutLayer(*this); }
  inline Base* clone_with_shared_params() { return clone(); }
  inline const Base& get_params_owner() const { return *this; }
  ...
  inline std::vector<const Parameters<Scalar>*> get_params() const { return std::vector<const Parameters<Scalar>*>(0); }
  inline std::vector<Parameters<Scalar>*> get_params() { return std::vector<Parameters<Scalar>*>(0); }
  inline void empty_cache() { dropout_mask = typename Base::Data(); }
  inline typename Base::Data pass_forward(typename Base::Data in, bool training) {
    ...
    if (training) {
      ...
      dropout_mask = in.random().unaryExpr(
          [this, scaling_factor](Scalar e) { return (Scalar)(e <= dropout_prob ? 0 : scaling_factor); });
