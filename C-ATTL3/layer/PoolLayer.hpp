...
class PoolLayer : public Layer<Scalar, Rank> {
 ...
 protected:
  ...
  inline PoolLayer(const typename Base::Dims& input_dims, std::size_t receptor_height, std::size_t receptor_width,
      ...
      : ext_input_dims(input_dims.template extend<3 - Rank>()),
        ext_output_dims(
            calculate_output_dims(ext_input_dims, receptor_height, receptor_width, vertical_stride, horizontal_stride)),
  ...
  /**
   * Differentiates the reduction function and returns the derivative of the
   * loss function w.r.t. the non-reduced patch.
   *
 ...
 private:
  ...
  inline static Dimensions<std::size_t, 3> calculate_output_dims(const Dimensions<std::size_t, 3>& input_dims,
                                                                 std::size_t receptor_height,
                                                                 std::size_t receptor_width,
                                                                 std::size_t vertical_stride,
                                                                 std::size_t horizontal_stride) {
    return {calculate_spatial_output_dim(input_dims(0), receptor_height, vertical_stride),
            calculate_spatial_output_dim(input_dims(1), receptor_width, horizontal_stride), input_dims(2)};
  }
  bool input_layer;
};
