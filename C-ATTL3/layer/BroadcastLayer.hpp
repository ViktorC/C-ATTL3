...
/**
 * A class template representing a broadcasting layer that repeats the contents
 * of its input tensors along its ranks.
 */
...
class BroadcastLayer : public Layer<Scalar, Rank> {
 ...
 public:
  ...
  inline BroadcastLayer(const typename Base::Dims& input_dims, const typename Base::Dims& broadcast)
    ...
    for (std::size_t i = 0; i < Rank; ++i) assert(broadcast(i) > 0);
  }
  inline Base* clone() const { return new BroadcastLayer(*this); }
