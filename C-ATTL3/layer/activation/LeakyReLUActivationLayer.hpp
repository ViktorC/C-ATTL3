...
/**
 * A class template representing a leaky rectified linear unit activation
 * function. Unlike traditional ReLU layers leaky ReLU layers do not set
 * negative elements of the input to 0 but scale them by a small constant alpha.
...
class LeakyReLUActivationLayer : public ActivationLayer<Scalar, Rank> {
 ...
 public:
  ...
  inline LeakyReLUActivationLayer(const typename Root::Dims& dims, Scalar alpha = 1e-1)
      : Base::ActivationLayer(dims), alpha(alpha) {}
  inline Root* clone() const { return new LeakyReLUActivationLayer(*this); }
  inline void empty_cache() { in_cache = typename Root::Data(); }
