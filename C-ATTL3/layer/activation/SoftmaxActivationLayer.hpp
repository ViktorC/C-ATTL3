...
/**
 * A class template for a softmax activation function layer. Unlike most other
 * activation functions, the softmax layer does not represent a simple
 * coefficient-wise function but a multivariate one. The per-sample sums of the
 * elements of the output tensor of the layer are always 1.
 *
...
class SoftmaxActivationLayer : public ActivationLayer<Scalar, Rank> {
 ...
 public:
   ...
   */
  inline SoftmaxActivationLayer(const typename Root::Dims& dims, Scalar epsilon = NumericUtils<Scalar>::EPSILON2)
      : Base::ActivationLayer(dims), epsilon(epsilon), conversion_dims(dims.template promote<>()) {}
  inline Root* clone() const { return new SoftmaxActivationLayer(*this); }
  inline void empty_cache() { out_mat_cache = Matrix<Scalar>(); }
  inline typename Root::Data pass_forward(typename Root::Data in, bool training) {
    ...
    /* First subtract the value of the greatest coefficient from each element
     * row-wise to avoid an overflow due to raising e to great powers. */
