...
/**
 * A class template representing the parametric Swish activation function with
 * learnable beta values.
 *
...
class PSwishActivationLayer : public ActivationLayer<Scalar, Rank> {
 ...
 public:
   ...
   * @param init_beta The initial factor by which the input of the sigmoid
   * factor of the Swish function is to be scaled.
   * @param beta_reg An optional regularization function to apply to the
   * parameters.
   * @param beta_clip The maximum allowed absolute parameter value. If it is 0
   * or less, no value clipping is performed.
   * @param beta_max_l1_norm The maximum allowed L1 beta value norm. If it is 0
   * or less, no L1 max norm constraint is enforced.
   * @param beta_max_l2_norm The maximum allowed L2 beta value norm. If it is 0
   * or less, no L2 max norm constraint is enforced.
   * @param beta_grad_clip The maximum allowed absolute parameter gradient. If
   ...
   * it is 0 or less, no L1 gradient max norm constraint is enforced.
   * @param beta_grad_max_l2_norm The maximum allowed L2 beta gradient norm. If
   * it is 0 or less, no L2 gradient max norm constraint is enforced.
   */
  inline PSwishActivationLayer(const typename Root::Dims& dims, Scalar init_beta = 1,
                               ParamRegSharedPtr<Scalar> beta_reg = nullptr, Scalar beta_clip = 0,
                               Scalar beta_max_l1_norm = 0, Scalar beta_max_l2_norm = 0, Scalar beta_grad_clip = 0,
                               Scalar beta_grad_max_l1_norm = 0, Scalar beta_grad_max_l2_norm = 0)
      : Base::ActivationLayer(
            dims, std::make_shared<StandardParameters<Scalar>>(
                      1, dims.get_volume(), true, std::make_shared<ConstantParameterInitialization<Scalar>>(init_beta),
                      beta_reg, beta_clip, beta_max_l1_norm, beta_max_l2_norm, beta_grad_clip, beta_grad_max_l1_norm,
  ...
  inline Root* clone() const { return new PSwishActivationLayer(*this); }
  inline Root* clone_with_shared_params() { return new PSwishActivationLayer(*this, true); }
  ...
  inline typename Root::Data pass_back(typename Root::Data out_grad) {
    ...
    Base::params->accumulate_grad(sig_out_mat_cache.cwiseProduct(one_min_sig_out_mat)
                                      .cwiseProduct(in_mat_cache)
                                      .cwiseProduct(in_mat_cache)
                                      .cwiseProduct(out_grad_mat)
                                      .colwise()
                                      .sum());
    ...
    Matrix<Scalar> prev_out_grad_mat =
        sig_out_mat_cache
            .cwiseProduct(
                ((one_min_sig_out_mat * Base::params->get_values().asDiagonal()).array() * in_mat_cache.array() + 1)
                    .matrix())
            .cwiseProduct(out_grad_mat);
