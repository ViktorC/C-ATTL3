...
class SwishActivationLayer : public ActivationLayer<Scalar, Rank> {
 ...
 public:
   ...
   * @param beta The factor by which the input of the sigmoid factor of the
   * Swish function is to be scaled.
   */
  inline SwishActivationLayer(const typename Root::Dims& dims, Scalar beta = 1)
      : Base::ActivationLayer(dims), beta(beta) {}
  inline Root* clone() const { return new SwishActivationLayer(*this); }
  ...
  inline typename Root::Data pass_back(typename Root::Data out_grad) {
    ...
    return sig_out_cache * ((sig_out_cache.constant(1) - sig_out_cache) * beta * in_cache + sig_out_cache.constant(1)) *
           out_grad;
  }
