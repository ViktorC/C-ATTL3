...
/**
 * A class template that represents a binary step activation function that
 * outputs either 1 or 0 based on the signum of its input. This function is not
 * differentiable.
 *
 * \f[
 *   f(x) = \begin{cases}
...
class BinaryStepActivationLayer : public ActivationLayer<Scalar, Rank> {
 ...
 public:
   ...
   */
  inline BinaryStepActivationLayer(const typename Root::Dims& dims) : Base::ActivationLayer(dims) {}
  inline Root* clone() const { return new BinaryStepActivationLayer(*this); }
