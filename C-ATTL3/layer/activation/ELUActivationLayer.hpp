...
/**
 * A class template representing an exponential linear unit (ELU) activation
 * function. ELUs apply an exponential (e based) function scaled by alpha to the
 * negative elements of the input. ELU layers are not differentiable.
 *
 * \f[
 *   f(x) = \begin{cases}
...
class ELUActivationLayer : public ActivationLayer<Scalar, Rank> {
 ...
 public:
  ...
  inline ELUActivationLayer(const typename Root::Dims& dims, Scalar alpha = 1e-1)
      : Base::ActivationLayer(dims), alpha(alpha), conversion_dims(dims.template promote<>()) {}
  inline Root* clone() const { return new ELUActivationLayer(*this); }
  ...
  inline typename Root::Data pass_forward(typename Root::Data in, bool training) {
    ...
    out_mat_cache = in_mat_cache.unaryExpr([this](Scalar e) { return (Scalar)(e >= 0 ? e : (alpha * (exp(e) - 1))); });
  ...
  inline typename Root::Data pass_back(typename Root::Data out_grad) {
    ...
    for (int i = 0; i < in_mat_cache.cols(); ++i) {
      for (int j = 0; j < in_mat_cache.rows(); ++j)
        prev_out_grad_mat(j, i) =
            (Scalar)((in_mat_cache(j, i) >= 0 ? 1 : (out_mat_cache(j, i) + alpha)) * out_grad_mat(j, i));
    }
