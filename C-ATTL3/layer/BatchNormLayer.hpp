...
class BatchNormLayer : public Layer<Scalar, Rank> {
 ...
 public:
   ...
   * @param gamma_reg An optional regularization function to apply to the
   * gammas.
   * @param gamma_clip The maximum allowed absolute gamma value. If it is 0 or
   * less, no value clipping is performed.
   * @param gamma_max_l1_norm The maximum allowed L1 gamma value norm. If it is
   * 0 or less, no L1 max norm constraint is enforced.
   * @param gamma_max_l2_norm The maximum allowed L2 gamma value norm. If it is
   * 0 or less, no L2 max norm constraint is enforced.
   * @param gamma_grad_clip The maximum allowed absolute gamma gradient. If it
   * is 0 or less, no gradient clipping is performed.
   * @param gamma_grad_max_l1_norm The maximum allowed L1 gamma gradient norm.
   * If it is 0 or less, no L1 gradient max norm constraint is enforced.
   * @param gamma_grad_max_l2_norm The maximum allowed L2 gamma gradient norm.
   ...
   * @param beta_clip The maximum allowed absolute beta value. If it is 0 or
   * less, no value clipping is performed.
   * @param beta_max_l1_norm The maximum allowed L1 beta value norm. If it is 0
   * or less, no beta L1 max norm constraint is enforced.
   * @param beta_max_l2_norm The maximum allowed L2 beta value norm. If it is 0
   * or less, no beta L2 max norm constraint is enforced.
   * @param beta_grad_clip The maximum allowed absolute beta gradient. If it is
   * 0 or less, no gradient clipping is performed.
   * @param beta_grad_max_l1_norm The maximum allowed L1 beta gradient norm. If
   * it is 0 or less, no beta L1 gradient max norm constraint is enforced.
   * @param beta_grad_max_l2_norm The maximum allowed L2 beta gradient norm. If
   * it is 0 or less, no beta L2 gradient max norm constraint is enforced.
   */
  inline BatchNormLayer(const typename Base::Dims& dims, Scalar norm_avg_decay = .1,
                        ...
                        Scalar gamma_grad_clip = 0, Scalar gamma_grad_max_l1_norm = 0,
                        Scalar gamma_grad_max_l2_norm = 0, ParamRegSharedPtr<Scalar> beta_reg = nullptr,
                        Scalar beta_clip = 0, Scalar beta_max_l1_norm = 0, Scalar beta_max_l2_norm = 0,
                        Scalar beta_grad_clip = 0, Scalar beta_grad_max_l1_norm = 0, Scalar beta_grad_max_l2_norm = 0)
      : owner(*this),
        dims(dims),
        ...
        memb_vec(channels) {
    assert(norm_avg_decay >= 0 && norm_avg_decay <= 1 && "norm avg decay must not be less than 0 or greater than 1");
    ...
    for (std::size_t i = 0; i < channels; ++i) {
      ...
      memb.gammas = std::make_shared<StandardParameters<Scalar>>(1, 1, true, gamma_init, gamma_reg, gamma_clip,
                                                                 gamma_max_l1_norm, gamma_max_l2_norm, gamma_grad_clip,
                                                                 gamma_grad_max_l1_norm, gamma_grad_max_l2_norm);
      memb.betas = std::make_shared<StandardParameters<Scalar>>(1, 1, true, beta_init, beta_reg, beta_clip,
                                                                beta_max_l1_norm, beta_max_l2_norm, beta_grad_clip,
  ...
  inline BatchNormLayer(const Self& layer, bool share_params = false)
    ...
    for (std::size_t i = 0; i < channels; ++i) {
      ...
      if (share_params) {
        ...
        memb1.avg_means = StdParamsSharedPtr(static_cast<StandardParameters<Scalar>*>(memb2.avg_means->clone()));
        memb1.avg_inv_sds = StdParamsSharedPtr(static_cast<StandardParameters<Scalar>*>(memb2.avg_inv_sds->clone()));
        memb1.gammas = StdParamsSharedPtr(static_cast<StandardParameters<Scalar>*>(memb2.gammas->clone()));
        memb1.betas = StdParamsSharedPtr(static_cast<StandardParameters<Scalar>*>(memb2.betas->clone()));
      }
  ...
  inline Base* clone() const { return new BatchNormLayer(*this); }
  inline Base* clone_with_shared_params() { return new BatchNormLayer(*this, true); }
  ...
  inline void empty_cache() {
    for (unsigned i = 0; i < memb_vec.size(); ++i) memb_vec[i].std_in_mat_cache = Matrix<Scalar>();
  }
 ...
 private:
  inline Matrix<Scalar> _pass_forward(MatrixMap<Scalar>& in, std::size_t i, bool training) {
    ...
    if (training) {
      ...
      if (memb.avgs_init) {
        memb.avg_means->set_values((1 - norm_avg_decay) * memb.avg_means->get_values().array() + norm_avg_decay * mean);
  ...
  inline Matrix<Scalar> _pass_back(MatrixMap<Scalar>& out_grad, std::size_t i) {
    ChannelSpecificMembers& memb = memb_vec[i];
    memb.gammas->accumulate_grad(Matrix<Scalar>::Constant(1, 1, out_grad.cwiseProduct(memb.std_in_mat_cache).sum()));
    memb.betas->accumulate_grad(Matrix<Scalar>::Constant(1, 1, out_grad.sum()));
    if (input_layer) return Matrix<Scalar>();
 ...
 public:
  ...
  inline BatchNormLayer(const typename Base::Dims& dims, Scalar norm_avg_decay = .1,
      ...
      : owner(*this),
        ...
        avgs_init(false),
        gammas(std::make_shared<StandardParameters<Scalar>>(
            1, dims.get_volume(), true, std::make_shared<OneParameterInitialization<Scalar>>(), gamma_reg, gamma_clip,
            gamma_max_l1_norm, gamma_max_l2_norm, gamma_grad_clip, gamma_grad_max_l1_norm, gamma_grad_max_l2_norm)),
        betas(std::make_shared<StandardParameters<Scalar>>(
            1, dims.get_volume(), true, std::make_shared<ZeroParameterInitialization<Scalar>>(), beta_reg, beta_clip,
            beta_max_l1_norm, beta_max_l2_norm, beta_grad_clip, beta_grad_max_l1_norm, beta_grad_max_l2_norm)),
        input_layer(false) {
    assert(norm_avg_decay >= 0 && norm_avg_decay <= 1 && "norm avg decay must not be less than 0 or greater than 1");
    assert(epsilon > 0 && "epsilon must be greater than 0");
  }
  ...
  inline void set_input_layer(bool input_layer) { this->input_layer = input_layer; }
  inline std::vector<Parameters<Scalar>*> get_params() {
    return std::vector<Parameters<Scalar>*>({avg_means.get(), avg_inv_sds.get(), gammas.get(), betas.get()});
  }
  inline std::vector<const Parameters<Scalar>*> get_params() const {
    return std::vector<const Parameters<Scalar>*>({avg_means.get(), avg_inv_sds.get(), gammas.get(), betas.get()});
  }
  inline void empty_cache() {
  ...
  inline typename Base::Data pass_forward(typename Base::Data in, bool training) {
    ...
    if (training) {
      ...
      if (avgs_init) {
        ...
        avg_inv_sds->set_values((1 - norm_avg_decay) * avg_inv_sds->get_values() + norm_avg_decay * inv_in_sd_cache);
      } else {
    ...
    Matrix<Scalar> out_mat = (in_mat * gammas->get_values().asDiagonal()).rowwise() + betas->get_values().row(0);
  ...
  inline typename Base::Data pass_back(typename Base::Data out_grad) {
    ...
    if (input_layer) return typename Base::Data();
    Matrix<Scalar> std_in_grad_mat = out_grad_mat * gammas->get_values().asDiagonal();
    Matrix<Scalar> prev_out_grad_mat =
        (((rows * std_in_grad_mat).rowwise() - std_in_grad_mat.colwise().sum()) -
         std_in_mat_cache * (std_in_mat_cache.cwiseProduct(std_in_grad_mat).colwise().sum().asDiagonal())) *
