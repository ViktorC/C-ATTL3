...
public:
	inline ConvKernelGPULayer(const typename Root::Dims& input_dims, std::size_t filters,
			...
			GPUParamRegSharedPtr<Scalar> bias_reg = nullptr) :
				GPUBase(input_dims, calculate_adjusted_output_dims(input_dims, filters, receptor_height,
						...
						input_dims.template extend<3 - Rank>(), calculate_output_dims(
								input_dims.template extend<3 - Rank>(), filters, receptor_height,
								receptor_width, vertical_padding, horizontal_padding, vertical_stride,
								horizontal_stride, vertical_dilation, horizontal_dilation),
		...
		assert(GPUBase::gpu_input_dims(0) + 2 * vertical_padding >=
				receptor_height + (receptor_height - 1) * vertical_dilation &&
				GPUBase::gpu_input_dims(1) + 2 * horizontal_padding >=
	...
	inline GPULayer<Scalar,Rank>* gpu_clone() const {
		return new ConvKernelGPULayer<Scalar,Rank>(*this);
	}
	...
	inline CuDNNTensor<Scalar> pass_forward(CuDNNTensor<Scalar> in, bool training) {
		...
		cudnnAssert(cudnnGetConvolutionForwardWorkspaceSize(CuDNNHandle::get_instance(), in_cache.desc(),
				weights.get_gpu_values().filter_desc(), conv_desc, out.desc(), conv_algo, &workspace_size));
		// Allocate the memory for the workspace required for the convolution.
		CUDAArray<Scalar> workspace(static_cast<std::size_t>(ceil(static_cast<Scalar>(workspace_size) /
				sizeof(Scalar))));
		// Perform the convolution.
		cudnnAssert(cudnnConvolutionForward(CuDNNHandle::get_instance(), &alpha, in_cache.desc(), in_cache.data(),
				weights.get_gpu_values().filter_desc(), weights.get_values().data(), conv_desc, conv_algo,
				workspace.data(), workspace_size, &beta, out.desc(), out.data()));
	...
	inline CuDNNTensor<Scalar> pass_back(CuDNNTensor<Scalar> out_grad) {
		...
		cudnnAssert(cudnnSetConvolution2dDescriptor(dconv_desc, vertical_padding, horizontal_padding,
		...
		cudnnAssert(cudnnGetConvolutionBackwardFilterWorkspaceSize(CuDNNHandle::get_instance(), in_cache.desc(),
				out_grad.desc(), dconv_desc, weights_grad.filter_desc(), dconv_filter_algo, &filter_workspace_size));
		// Allocate the memory required for the backwards filter convolution on the device.
		CUDAArray<Scalar> filter_workspace(static_cast<std::size_t>(ceil(static_cast<Scalar>(filter_workspace_size) /
				sizeof(Scalar))));
		// Perform the backwards filter convolution.
		cudnnAssert(cudnnConvolutionBackwardFilter(CuDNNHandle::get_instance(), &alpha, in_cache.desc(),
				in_cache.data(), out_grad.desc(), out_grad.data(), dconv_desc, dconv_filter_algo,
				filter_workspace.data(), filter_workspace_size, &beta, weights_grad.filter_desc(),
				weights_grad.data()));
		// Free up resources.
		cudnnAssert(cudnnDestroyConvolutionDescriptor(dconv_desc));
		// Perform the backwards bias convolution.
		cudnnAssert(cudnnConvolutionBackwardBias(CuDNNHandle::get_instance(), &alpha, out_grad.desc(),
				out_grad.data(), &beta, bias_grad.desc(), bias_grad.data()));
		...
		cudnnAssert(cudnnGetConvolutionBackwardDataWorkspaceSize(CuDNNHandle::get_instance(),
				weights.get_gpu_values().filter_desc(), out_grad.desc(), dconv_desc, prev_out_grad.desc(),
				dconv_data_algo, &data_workspace_size));
		// Allocate the memory required for the backwards data convolution on the device.
		CUDAArray<Scalar> data_workspace(static_cast<std::size_t>(ceil(static_cast<Scalar>(data_workspace_size) /
				sizeof(Scalar))));
		// Perform the backwards data convolution.
		cudnnAssert(cudnnConvolutionBackwardData(CuDNNHandle::get_instance(), &alpha,
				weights.get_gpu_values().filter_desc(), weights.get_gpu_values().data(), out_grad.desc(),
...
private:
	inline static Dimensions<std::size_t,3> calculate_output_dims(const Dimensions<std::size_t,3>& input_dims,
		...
		CuDNNTensor<Scalar>::create_filter_descriptor(filter_desc, filters, receptor_height,
				receptor_width, input_dims(2));
		// Create and set up the convolution descriptor.
		cudnnConvolutionDescriptor_t conv_desc;
		cudnnAssert(cudnnCreateConvolutionDescriptor(&conv_desc));
		cudnnAssert(cudnnSetConvolution2dDescriptor(conv_desc, vertical_padding, horizontal_padding,
