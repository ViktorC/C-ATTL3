  ...
  inline DenseKernelGPULayer(const typename Root::Dims& input_dims, std::size_t output_size,
      ...
      : Base(input_dims, {output_size}, input_dims.template extend<3 - Rank>(), {output_size},
             std::make_shared<StandardGPUParameters<Scalar>>(
                 output_size, input_dims.template extend<3 - Rank>()(0), input_dims.template extend<3 - Rank>()(1),
                 input_dims.template extend<3 - Rank>()(2), true, weight_init, weight_reg),
             std::make_shared<StandardGPUParameters<Scalar>>(
                 1, output_size, 1, 1, true, std::make_shared<ZeroGPUParameterInitialization<Scalar>>(), bias_reg)) {}
