...
public:
	inline SoftmaxActivationGPULayer(const typename Root::Dims& dims) :
			Base(dims),
			alpha(1),
			beta(0) { }
	inline GPULayer<Scalar,Rank>* gpu_clone() const {
		return new SoftmaxActivationGPULayer<Scalar,Rank>(*this);
	}
	...
	inline CuDNNTensor<Scalar> pass_forward(CuDNNTensor<Scalar> in, bool training) {
		...
		assert(in.samples() > 0);
		cudnnAssert(cudnnSoftmaxForward(CuDNNHandle::get_instance(), CUDNN_SOFTMAX_ACCURATE,
				CUDNN_SOFTMAX_MODE_INSTANCE, &alpha, in.desc(), in.data(), &beta, in.desc(), in.data()));
		if (training)
			out_cache = in;
		return in;
	}
	inline CuDNNTensor<Scalar> pass_back(CuDNNTensor<Scalar> out_grad) {
		...
		cudnnAssert(cudnnSoftmaxBackward(CuDNNHandle::get_instance(), CUDNN_SOFTMAX_ACCURATE,
				CUDNN_SOFTMAX_MODE_INSTANCE, &alpha, out_cache.desc(), out_cache.data(), out_grad.desc(),
				out_grad.data(), &beta, out_grad.desc(), out_grad.data()));
		return out_grad;
	}
private:
