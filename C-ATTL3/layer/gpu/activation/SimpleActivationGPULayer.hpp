...
public:
	inline SimpleActivationGPULayer(const typename Root::Dims& dims, cudnnActivationMode_t act_mode,
			Scalar coeff = 0) :
				Base(dims),
				coeff(coeff),
				alpha(1),
	...
	inline virtual ~SimpleActivationGPULayer() {
	...
	inline CuDNNTensor<Scalar> pass_forward(CuDNNTensor<Scalar> in, bool training) {
		...
		CuDNNTensor<Scalar> out(in_cache.samples(), in_cache.height(), in_cache.width(),
				in_cache.channels());
		cudnnAssert(cudnnActivationForward(CuDNNHandle::get_instance(), act_desc, &alpha, in_cache.desc(),
				in_cache.data(), &beta, out.desc(), out.data()));
		if (training)
			out_cache = out;
		return out;
	}
	inline CuDNNTensor<Scalar> pass_back(CuDNNTensor<Scalar> out_grad) {
		...
		cudnnAssert(cudnnActivationBackward(CuDNNHandle::get_instance(), act_desc, &alpha, out_cache.desc(),
				out_cache.data(), out_grad.desc(), out_grad.data(), in_cache.desc(), in_cache.data(), &beta,
				out_grad.desc(), out_grad.data()));
		return out_grad;
	}
private:
