...
/**
 * A class template representing a reshaping layer that outputs a reshaped copy
 * of the input tensor with the same volume. The data backing the tensor is not
 * shifted in any way.
 */
...
class ReshapeLayer : public Layer<Scalar, Rank> {
 ...
 public:
   ...
   * @param output_dims The dimensions of the reshaped tensor. The output tensor
   * must have the same volume as the input tensor.
   */
  ...
  inline Base* clone() const { return new ReshapeLayer(*this); }
