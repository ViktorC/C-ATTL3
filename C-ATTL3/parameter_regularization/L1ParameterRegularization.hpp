...
class L1ParameterRegularization : public ParameterRegularization<Scalar> {
 public:
   ...
   */
  inline L1ParameterRegularization(Scalar lambda = 1e-2) : lambda(lambda) {}
  inline Scalar function(const Matrix<Scalar>& params) const { return params.cwiseAbs().sum() * lambda; }
