...
class L2ParameterRegularization : public ParameterRegularization<Scalar> {
 public:
   ...
   */
  inline L2ParameterRegularization(Scalar lambda = 1e-2) : lambda(lambda) {}
  inline Scalar function(const Matrix<Scalar>& params) const { return params.squaredNorm() * ((Scalar).5 * lambda); }
  inline Matrix<Scalar> d_function(const Matrix<Scalar>& params) const { return params * lambda; }
