...
/**
 * A template class representing the hinge loss function. This class assumes the
 * objectives for each sample (and time step) to be a one-hot vector (tensor
 * rank).
 *
 ...
 protected:
  inline ColVector<Scalar> _function(typename Root::Data out, typename Root::Data obj) const {
    ...
    for (int i = 0; i < rows; ++i) {
      ...
      for (int j = 0; j < cols; ++j) {
        if (j == correct_class_ind) continue;
