...
/**
 * An enumeration for the different objective types to use for the IMDB data
 * set.
 */
...
class IMDBDataProvider : public JointFileDataProvider<Scalar, 1, true> {
 ...
 public:
  ...
  /**
   * @param pos_reviews_folder_path The path to the folder containing the
   * positive reviews (without a trailing path separator).
   * @param neg_reviews_folder_path The pathe to the folder containing the
   * negative reveies (without a trailing path separator).
   * @param vocab A shared pointer to the vocabulary to use.
   * @param seq_length The sequence length to trim or pad the data to so as to
   * enable batch training. If it is set to 0, no sequence trimming or padding
   * is to be performed (which is likely to make batch training impossible).
   */
  ...
  inline static VocabSharedPtr build_vocab(std::string vocab_path) {
    ...
    std::size_t index = 2;
    std::string word;
    while (vocab_stream >> word) vocab.emplace(std::make_pair(word, index++));
 ...
 protected:
  ...
  inline static void read_files_in_dir(std::string dir_path, std::vector<std::string>& file_names) {
    ...
    while ((dir_ent_ptr = readdir(dir_ptr))) file_names.push_back(dir_path + "/" + std::string(dir_ent_ptr->d_name));
    closedir(dir_ptr);
  }
  /**
  ...
  inline static void clean_document(std::stringstream& document) {
    ...
    std::transform(doc_string.begin(), doc_string.end(), doc_string.begin(), static_cast<int (*)(int)>(std::tolower));
  ...
  inline DataPair<Scalar, 1, true> _get_data(const std::string& file_name, std::ifstream& file_stream,
    ...
    std::string rating_string = file_name.substr(last_under_score + 1, last_period - last_under_score - 1);
    ...
    if (fixed_seq_length) {
      for (; time_step < seq_length; ++time_step) obs(0u, time_step, +PAD_IND) = (Scalar)1;
    } else {
