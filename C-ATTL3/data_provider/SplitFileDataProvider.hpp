...
/**
 * An abstract class template for a data provider backed by an arbitrary number
 * of file pairs containing the separated observations and the objectives.
 * Implementations are responsible for specifying the dimensions of both the
 * observations and the objectives, for reading batches of observation-objective
 * pairs from the file, and for skipping arbitrary number of data instances.
 */
...
class SplitFileDataProvider : public DataProvider<Scalar, Rank, Sequential> {
 ...
 public:
  ...
  inline bool has_more() {
    if (current_file_stream_pair_has_more()) return true;
  ...
  inline DataPair<Scalar, Rank, Sequential> get_data(std::size_t batch_size) {
    ...
    DataPair<Scalar, Rank, Sequential> data_pair =
        _get_data(current_file_pair.first, current_file_stream_pair.first, current_file_pair.second,
                  current_file_stream_pair.second, batch_size);
    ...
    while (data_pair.first.dimension(0) < batch_size && has_more()) {
      ...
      DataPair<Scalar, Rank, Sequential> add_data_pair =
          _get_data(new_current_file_pair.first, current_file_stream_pair.first, new_current_file_pair.second,
                    current_file_stream_pair.second, batch_size - data_pair.first.dimension(0));
  ...
  inline void skip(std::size_t instances) {
    if (!has_more()) return;
    std::size_t skipped = _skip(current_file_stream_pair.first, current_file_stream_pair.second, instances);
    while (skipped < instances && has_more())
      skipped += _skip(current_file_stream_pair.first, current_file_stream_pair.second, instances - skipped);
  }
 ...
 protected:
  inline SplitFileDataProvider(const typename Base::Dims& obs_dims, const typename Base::Dims& obj_dims,
      ...
      : obs_dims(obs_dims), obj_dims(obj_dims), file_pairs(dataset_path_pairs), current_file_pair_ind(0) {
  ...
  /**
   * It sets the positions of the file streams to the beginning of the
   * observation data set and the objective data set respectively.
   *
   * @param obs_file_stream A reference to the file stream to a file containing
   * observations.
   * @param obj_file_stream A reference to the file stream to a file containing
   * objectives.
   */
  ...
  /**
   * It reads at most the specified number of observations from the
   * observation-file and at most the specified number of objectives from the
   * objective-file. The file streams can be expected not to have any of their
   * fail flags set initially and to have at least 1 more character left to read
   * in each.
   *
   ...
   */
  virtual DataPair<Scalar, Rank, Sequential> _get_data(const std::string& obs_file, std::ifstream& obs_file_stream,
                                                       const std::string& obj_file, std::ifstream& obj_file_stream,
                                                       std::size_t batch_size) = 0;
  /**
   * Skips at most the specified number of instances in the data streams. The
   * file streams can be expected not to have any of their fail flags set
   * initially.
   *
   * @param obs_file_stream A reference to the file stream to a file containing
   * observations.
   ...
   * @return The number of actual data points skipped. It may be less than the
   * specified amount if there are fewer remaining instances in the data
   * streams.
   */
  virtual std::size_t _skip(std::ifstream& obs_file_stream, std::ifstream& obj_file_stream, std::size_t instances) = 0;
