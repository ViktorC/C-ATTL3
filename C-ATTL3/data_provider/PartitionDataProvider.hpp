...
/**
 * A wrapper class template for data providers associated with continuous
 * partitions of other data providers. It enables the partitioning of a data
 * provider into training and test data providers by mapping two contiguous
 * blocks of its data to two PartitionedDataProvider instances.
 */
...
class PartitionDataProvider : public DataProvider<Scalar, Rank, Sequential> {
 ...
 public:
  inline PartitionDataProvider(Base& orig_provider, std::size_t offset, std::size_t length)
      : orig_provider(orig_provider), offset(offset), length(length) {
    assert(length > 0);
    reset();
  }
  inline const typename Base::Dims& get_obs_dims() const { return orig_provider.get_obs_dims(); }
  inline const typename Base::Dims& get_obj_dims() const { return orig_provider.get_obj_dims(); }
  inline bool has_more() { return instances_read < length && orig_provider.has_more(); }
  ...
  inline void skip(std::size_t instances) { orig_provider.skip(instances); }
