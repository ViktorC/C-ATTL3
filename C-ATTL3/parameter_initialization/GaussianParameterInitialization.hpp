...
/**
 * An abstract class template representing a random parameter initialization
 * method that samples from a Gaussian distribution.
 */
 ...
 public:
  ...
  inline virtual void apply(Matrix<Scalar>& params) const {
    ...
    for (int i = 0; i < cols; ++i) {
      for (int j = 0; j < rows; ++j) params(j, i) = dist(gen);
    }
  }
