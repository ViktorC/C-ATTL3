...
class OrthogonalParameterInitialization : public GaussianParameterInitialization<Scalar> {
 public:
   ...
   */
  inline OrthogonalParameterInitialization(Scalar sd = 1) : GaussianParameterInitialization<Scalar>(0, sd) {}
  inline void apply(Matrix<Scalar>& params) const {
    ...
    SVD<Scalar> svd;
    params.block(0, 0, rows, cols) =
        more_rows ? svd.compute(params, SVDOptions::ComputeFullU).matrixU().block(0, 0, rows, cols)
