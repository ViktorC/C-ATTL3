...
/**
 * An alias for a shared pointer to a WeightInitialization implementation
 * instance of an arbitrary scalar type.
 */
template <typename Scalar>
using ParamInitSharedPtr = std::shared_ptr<ParameterInitialization<Scalar>>;

/**
 * An alias for a shared pointer to a regularization penalty of an arbitrary
 * scalar type.
 */
template <typename Scalar>
...
class StandardParameters : public Parameters<Scalar> {
 public:
   ...
   * parameters do not maintain gradient information and are thus not
   * regularizable.
   * @param init The parameter value initialization. If it is a null pointer,
   * the values will not be initialized.
   * @param reg The optional regularization to use on the values of the
   * parameter matrix. If it is a null pointer, no regularization is applied.
   ...
   * @param grad_clip The maximum allowed absolute parameter gradient. If it is
   * 0 or less, no gradient clipping is performed.
   * @param grad_max_l1_norm The maximum allowed L1 parameter gradient norm. If
   * it is 0 or less, no L1 gradient max norm constraint is enforced.
   * @param grad_max_l2_norm The maximum allowed L2 parameter gradient norm. If
  ...
  inline Parameters<Scalar>* clone() const { return new StandardParameters<Scalar>(*this); }
  ...
  inline const Matrix<Scalar>& get_values() const { return values; }
  ...
  inline const Matrix<Scalar>& get_grad() const { return grad; }
  ...
  inline void regularize() {
    if (optimizable && param_reg) accumulate_grad(param_reg->d_function(values));
  }
 ...
 protected:
  /**
   * It clips the values of the matrix falling outside the interval [-clip,
   * clip].
   *
  ...
  inline static void enforce_clip_constraint(Matrix<Scalar>& matrix, Scalar clip) {
    if (NumericUtils<Scalar>::decidedly_greater(clip, (Scalar)0)) {
      matrix = matrix.unaryExpr([clip](Scalar m) { return m > clip ? clip : (m < -clip ? -clip : m); });
    }
  }
  /**
  ...
  inline static void enforce_l1_norm_constraint(Matrix<Scalar>& matrix, Scalar max_l1_norm) {
    if (NumericUtils<Scalar>::decidedly_greater(max_l1_norm, (Scalar)0)) {
      ...
      if (l1_norm > max_l1_norm) matrix *= (max_l1_norm / l1_norm);
    }
  }
  /**
  ...
  inline static void enforce_l2_norm_constraint(Matrix<Scalar>& matrix, Scalar max_l2_norm) {
    if (NumericUtils<Scalar>::decidedly_greater(max_l2_norm, (Scalar)0)) {
      ...
      if (l2_norm > max_l2_norm) matrix *= (max_l2_norm / l2_norm);
    }
  }
