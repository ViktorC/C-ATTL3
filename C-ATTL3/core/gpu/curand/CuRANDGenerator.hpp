...
template <typename Scalar>
__inline__ NormalGenerationRoutine<Scalar> normal_gen_routine() {
  return &curandGenerateNormalDouble;
}
template <>
__inline__ NormalGenerationRoutine<float> normal_gen_routine() {
...
template <typename Scalar>
__inline__ UniformGenerationRoutine<Scalar> uniform_gen_routine() {
  return &curandGenerateUniformDouble;
}
template <>
__inline__ UniformGenerationRoutine<float> uniform_gen_routine() {
...
/**
 * A template class for generating normally or uniformly distributed random
 * numbers using the cuRAND generator.
 */
template <typename Scalar>
class CuRANDGenerator {
 public:
  static constexpr curandRngType_t RNG_TYPE = CURAND_RNG_PSEUDO_DEFAULT;
  inline CuRANDGenerator() : gen() { curandAssert(curandCreateGenerator(&gen, RNG_TYPE)); }
  inline ~CuRANDGenerator() { curandAssert(curandDestroyGenerator(gen)); }
  /**
