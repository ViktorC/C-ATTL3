 ...
 public:
  ...
  inline typename Base::Data pass_forward(typename Base::Data in, bool training) {
    ...
    Tensor<Scalar, 4> out_extended =
        pass_forward(CuDNNTensor<Scalar>(TensorMap<Scalar, 4>(in.data(), in_gpu_dims)), training);
  ...
  inline typename Base::Data pass_back(typename Base::Data out_grad) {
    ...
    out_gpu_dims(0) = rows;
    Tensor<Scalar, 4> prev_out_grad_extended = pass_back(TensorMap<Scalar, 4>(out_grad.data(), out_gpu_dims));
