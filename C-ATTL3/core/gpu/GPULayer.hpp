...
/**
 * An alias for a shared pointer to a GPUParameters instance.
 */
template <typename Scalar>
using GPUParamsSharedPtr = std::shared_ptr<GPUParameters<Scalar>>;
...
class GPULayer : public virtual Layer<Scalar, Rank> {
 ...
 public:
  ...
  virtual CuDNNTensor<Scalar> gpu_pass_forward(CuDNNTensor<Scalar> in, bool training) = 0;
  virtual CuDNNTensor<Scalar> gpu_pass_back(CuDNNTensor<Scalar> out_grad) = 0;
  inline virtual Layer<Scalar, Rank>* clone() const { return gpu_clone(); }
  inline virtual Layer<Scalar, Rank>* clone_with_shared_params() { return gpu_clone_with_shared_params(); }
  inline typename Base::Data pass_forward(typename Base::Data in, bool training) {
    ...
    Tensor<Scalar, 4> out_extended =
        gpu_pass_forward(CuDNNTensor<Scalar>(TensorMap<Scalar, 4>(in.data(), in_gpu_dims)), training);
  ...
  inline typename Base::Data pass_back(typename Base::Data out_grad) {
    ...
    out_gpu_dims(0) = rows;
    Tensor<Scalar, 4> prev_out_grad_extended = gpu_pass_back(TensorMap<Scalar, 4>(out_grad.data(), out_gpu_dims));
