...
#define cublasErrorCheck(status) \
  { _cublas_error_check(status, __FILE__, __LINE__); }
#define cublasAssert(status) \
  { _cublas_assert(status, __FILE__, __LINE__); }
...
class CuBLASError : public std::runtime_error {
 public:
  ...
  CuBLASError(cublasStatus_t status, const char* file, int line)
      : std::runtime_error("cuBLAS Error: " + cublas_status_to_string(status) + "; File: " + std::string(file) +
...
__inline__ void _cublas_error_check(cublasStatus_t status, const char* file, int line) {
  if (status != CUBLAS_STATUS_SUCCESS) throw CuBLASError(status, file, line);
}
