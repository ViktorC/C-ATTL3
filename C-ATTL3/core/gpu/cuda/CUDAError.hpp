...
#define cudaErrorCheck(code) \
  { _cuda_error_check(__FILE__, __LINE__, code); }
#define cudaAssert(code) \
  { _cuda_assert(__FILE__, __LINE__, code); }
...
class CUDAError : public std::runtime_error {
 public:
  ...
  CUDAError(cudaError_t code, const char* file, int line)
      : std::runtime_error("CUDA Error: " + std::string(cudaGetErrorString(code)) + "; File: " + std::string(file) +
...
__inline__ void _cuda_error_check(const char* file, int line, cudaError_t code = cudaGetLastError()) {
  if (code != cudaSuccess) throw CUDAError(code, file, line);
}
