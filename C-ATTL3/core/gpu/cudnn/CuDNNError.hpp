...
#define cudnnErrorCheck(status) \
  { _cudnn_error_check(status, __FILE__, __LINE__); }
#define cudnnAssert(status) \
  { _cudnn_assert(status, __FILE__, __LINE__); }
...
class CuDNNError : public std::runtime_error {
 public:
  ...
  CuDNNError(cudnnStatus_t status, const char* file, int line)
      : std::runtime_error("cuDNN Error: " + std::string(cudnnGetErrorString(status)) + "; File: " + std::string(file) +
...
__inline__ void _cudnn_error_check(cudnnStatus_t status, const char* file, int line) {
  if (status != CUDNN_STATUS_SUCCESS) throw CuDNNError(status, file, line);
}
