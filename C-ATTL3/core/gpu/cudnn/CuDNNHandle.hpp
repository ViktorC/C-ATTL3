...
public:
	...
	inline void conv2d_output_dims(std::size_t input_height, std::size_t input_width, std::size_t input_channels,
		...
		CuDNNTensor<Scalar>::create_tensor_descriptor(input_desc, 1, input_height, input_width, input_channels);
		// Create and set up the filter descriptor.
		cudnnFilterDescriptor_t filter_desc;
		CuDNNFilter<Scalar>::create_filter_descriptor(filter_desc, filters, receptor_height,
				receptor_width, input_channels);
		...
		CuDNNFilter<Scalar>::destroy_filter_descriptor(filter_desc);
	 ...
	 */
	inline void convolution2d_fwd(const CuDNNTensor<Scalar>& input, const CuDNNFilter<Scalar>& filter,
		...
		cudnnAssert(cudnnGetConvolutionForwardAlgorithm(handle, input.desc(), filter.filter_desc(), conv_desc,
		...
		cudnnAssert(cudnnGetConvolutionForwardWorkspaceSize(handle, input.desc(), filter.filter_desc(), conv_desc,
				output.desc(), conv_algo, &workspace_size));
		// Allocate the memory for the workspace required for the convolution.
		CUDAArray<Scalar> workspace(static_cast<std::size_t>(
				ceil(static_cast<Scalar>(workspace_size) / sizeof(Scalar))));
		// Perform the convolution.
		cudnnAssert(cudnnConvolutionForward(handle, &alpha, input.desc(), input.data(), filter.filter_desc(),
				filter.data(), conv_desc, conv_algo, workspace.data(), workspace_size, &beta, output.desc(),
				output.data()));
	...
	inline void convolution2d_bwd(const CuDNNTensor<Scalar>& input, const CuDNNTensor<Scalar>& out_grad,
			const CuDNNFilter<Scalar>& filter, const CuDNNTensor<Scalar>& bias, std::size_t vertical_padding,
			std::size_t horizontal_padding, std::size_t vertical_stride, std::size_t horizontal_stride,
			std::size_t vertical_dilation, std::size_t horizontal_dilation, /* out */ CuDNNTensor<Scalar>& prev_out_grad,
			/* out */ CuDNNFilter<Scalar>& filter_grad, /* out */ CuDNNTensor<Scalar>& bias_grad) const {
		...
		cudnnAssert(cudnnGetConvolutionBackwardDataAlgorithm(handle, filter.filter_desc(), out_grad.desc(), dconv_desc,
		...
		cudnnAssert(cudnnGetConvolutionBackwardFilterAlgorithm(handle, input.desc(), out_grad.desc(), dconv_desc,
				filter_grad.filter_desc(), CUDNN_CONVOLUTION_BWD_FILTER_PREFER_FASTEST, 0, &dconv_filter_algo));
		...
		cudnnAssert(cudnnGetConvolutionBackwardDataWorkspaceSize(handle, filter.filter_desc(), out_grad.desc(), dconv_desc,
		...
		cudnnAssert(cudnnGetConvolutionBackwardFilterWorkspaceSize(handle, input.desc(), out_grad.desc(), dconv_desc,
				filter_grad.filter_desc(), dconv_filter_algo, &filter_workspace_size));
		// Allocate the memory required for the backwards data convolution on the device.
		CUDAArray<Scalar> data_workspace(static_cast<std::size_t>(
				ceil(static_cast<Scalar>(data_workspace_size) / sizeof(Scalar))));
		cudaAssert(cudaMalloc(&data_workspace, data_workspace_size));
		// Perform the backwards data convolution.
		cudnnAssert(cudnnConvolutionBackwardData(handle, &alpha, filter.filter_desc(), filter.data(), out_grad.desc(),
				out_grad.data(), dconv_desc, dconv_data_algo, data_workspace.data(), data_workspace_size, &beta,
				prev_out_grad.desc(), prev_out_grad.data()));
		// Allocate the memory required for the backwards filter convolution on the device.
		CUDAArray<Scalar> filter_workspace(static_cast<std::size_t>(
				ceil(static_cast<Scalar>(filter_workspace_size) / sizeof(Scalar))));
		...
		cudnnAssert(cudnnConvolutionBackwardFilter(handle, &alpha, input.desc(), input.data(), out_grad.desc(),
				out_grad.data(), dconv_desc, dconv_filter_algo, filter_workspace.data(), filter_workspace_size, &beta,
				filter_grad.filter_desc(), filter_grad.data()));
		// Free up resources.
	...
	inline void dropout_fwd(const CuDNNTensor<Scalar>& input, Scalar dropout,
			CUDAArray<Scalar>& state, /* in/out */ CUDAArray<Scalar>& reserve,
			/* out */ CuDNNTensor<Scalar>& output) const {
	...
	inline void dropout_bwd(const CuDNNTensor<Scalar>& out_grad, Scalar dropout, CuDNNTensor<Scalar>& state,
			CUDAArray<Scalar>& reserve, /* out */ CUDAArray<Scalar>& prev_out_grad) const {
		cudnnDropoutDescriptor_t dropout_desc;
		cudnnAssert(cudnnCreateDropoutDescriptor(&dropout_desc));
		cudnnAssert(cudnnSetDropoutDescriptor(dropout_desc, handle, (float) dropout, state.data(),
				state.size() * sizeof(Scalar), 0));
