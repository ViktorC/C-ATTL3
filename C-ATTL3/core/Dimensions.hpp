...
/**
 * NOTE: Expression templates are rather unwarranted for this class as its
 * objects are unlikely to be used in complex expressions. They have only been
 * implemented for the sake of practical learning.
 */
...
/**
 * A class representing dimensions along one or more ranks. It can describe the
 * dimensionality of tensors of arbitrary ranks.
 */
...
class Dimensions : public DimExpression<Dimensions<IndexType, Rank>, IndexType, Rank> {
 ...
 public:
  inline Dimensions() : values(Rank, 1) {}
  inline Dimensions(const std::initializer_list<IndexType>& values) : Dimensions() {
    assert(values.size() <= Rank);
    std::copy(values.begin(), values.end(), this->values.begin());
  }
  inline Dimensions(const std::array<IndexType, Rank>& array) : Dimensions() {
    std::copy(array.begin(), array.end(), values.begin());
  }
  template <typename OtherDerived>
  inline Dimensions(const DimExpression<OtherDerived, IndexType, Rank>& dims) : Dimensions() {
    for (std::size_t i = 0; i < Rank; ++i) values[i] = dims(i);
  }
  /**
   * A constant method that returns a copy of the instance with n additional
   * ranks prepended to it.
   *
   * @return A new Dimensions instance with additional n ranks of size 1
   * prepended to it.
   */
  ...
  /**
   * A constant method that returns a copy of the instance without the n
   * most-significant ranks.
   *
  ...
  /**
   * A constant method that returns a copy of the instance with n ranks appended
   * to it.
   *
   * @return A new Dimensions instance with additional n ranks with size 1
   * appended to it.
   */
  template <std::size_t Ranks = 1>
  ...
  /**
   * A simple constant method that returns a copy of the numeral representing
   * the number of dimensions along a given rank.
   *
  ...
  inline IndexType operator()(std::size_t i) const {
    if (i < 0 || i >= Rank) throw std::invalid_argument("illegal index value: " + std::to_string(i));
    return values[i];
  }
  /**
...
/**
 * An expression representing an operation between a numeral and all ranks of a
 * dimension expression.
 */
template <typename IndexType, std::size_t Rank, typename LhsExpr, typename OpType>
class UnaryDimExpression : public DimExpression<UnaryDimExpression<IndexType, Rank, LhsExpr, OpType>, IndexType, Rank> {
 public:
  inline UnaryDimExpression(const LhsExpr& lhs, const IndexType& rhs) : lhs(lhs), rhs(rhs){};
  inline IndexType operator()(std::size_t i) const {
    if (i < 0 || i >= Rank) throw std::invalid_argument("illegal index value: " + std::to_string(i));
...
/**
 * An expression representing an operation between a numeral and a single rank
...
/**
 * An expression representing an operation between two dimension expressions of
 * the same rank.
 */
...
class BinaryDimExpression
 ...
 public:
  inline BinaryDimExpression(const LhsExpr& lhs, const RhsExpr& rhs) : lhs(lhs), rhs(rhs){};
  inline IndexType operator()(std::size_t i) const {
    if (i < 0 || i >= Rank) throw std::invalid_argument("illegal index value: " + std::to_string(i));
...
/**
 * An expression representing an operation along a single rank of two dimension
 * expressions of the same rank.
 */
template <typename IndexType, std::size_t Rank, typename LhsExpr, typename RhsExpr, typename OpType>
...
class SumOp {
 public:
  inline static Operand apply(Operand lhs, Operand rhs) { return lhs + rhs; }
};
template <typename Operand>
class SubOp {
 public:
...
class DimExpression {
  ...
  template <typename OtherDerived>
  using Other = DimExpression<OtherDerived, IndexType, Rank>;

 public:
  inline operator Derived&() { return static_cast<Derived&>(*this); }
  inline operator const Derived&() const { return static_cast<const Derived&>(*this); }
  /**
   * Evaluates the expression along the given rank.
   *
   * @param i The index of the single rank along which the expression is to be
   * evaluated.
   * @return The result of the expression evaluated along the i-th rank.
   */
  inline IndexType operator()(std::size_t i) const { return static_cast<const Derived&>(*this)(i); }
  /**
  ...
  inline IndexType get_volume() const {
    int volume = 1;
    for (std::size_t i = 0; i < Rank; ++i) volume *= (*this)(i);
    return volume;
  }
  /**
   ...
   * expression.
   */
  inline Dimensions<IndexType, Rank> eval() { return Dimensions<IndexType, Rank>(*this); }
  /**
  ...
  inline std::string to_string() const {
    ...
    for (std::size_t i = 1; i < Rank; ++i) strm << "," << std::to_string((*this)(i));
    strm << "]";
    return strm.str();
  }
  /**
   ...
   * @return An expression representing the addition of the specified value to
   * the specified rank of the dimension expression.
   */
  inline UnaryRankWiseDimExpression<IndexType, Rank, Self, SumOp<IndexType>> add_along_rank(const IndexType& n,
   ...
   * @return An expression representing the subtraction of the specified value
   * from the specified rank of the dimension expression.
   */
  inline UnaryRankWiseDimExpression<IndexType, Rank, Self, SubOp<IndexType>> subtract_along_rank(
      const IndexType& n, std::size_t rank) const {
   ...
   * @return An expression representing the multiplication of the specified rank
   * of the dimension expression by the specified value.
   */
  inline UnaryRankWiseDimExpression<IndexType, Rank, Self, MulOp<IndexType>> multiply_along_rank(
      const IndexType& n, std::size_t rank) const {
   ...
   * @return An expression representing the division of the specified rank of
   * the dimension expression by the specified value.
   */
  inline UnaryRankWiseDimExpression<IndexType, Rank, Self, DivOp<IndexType>> divide_along_rank(const IndexType& n,
                                                                                               std::size_t rank) const {
   ...
   * dimension to the instance on which the method is invoked along the
   * specified rank.
   */
  template <typename OtherDerived>
  inline BinaryRankWiseDimExpression<IndexType, Rank, Self, Other<OtherDerived>, SumOp<IndexType>> add_along_rank(
      const Other<OtherDerived>& dims, std::size_t rank) const {
    return BinaryRankWiseDimExpression<IndexType, Rank, Self, Other<OtherDerived>, SumOp<IndexType>>(*this, dims, rank);
  }
  /**
   * @param dims The dimension expression to subtract.
   * @param rank The rank along which the dimension expression is to be
   * subtracted from the instance on which the method is invoked.
   * @return An expression representing the subtraction of the specified
   * expression dimension from the instance on which the method is invoked along
   * the specified rank.
   */
  template <typename OtherDerived>
  inline BinaryRankWiseDimExpression<IndexType, Rank, Self, Other<OtherDerived>, SubOp<IndexType>> subtract_along_rank(
      const Other<OtherDerived>& dims, std::size_t rank) const {
    return BinaryRankWiseDimExpression<IndexType, Rank, Self, Other<OtherDerived>, SubOp<IndexType>>(*this, dims, rank);
  }
  template <typename OtherDerived>
  inline BinaryDimExpression<IndexType, Rank, Self, Other<OtherDerived>, SumOp<IndexType>> operator+(
      const Other<OtherDerived>& dims) const {
    return BinaryDimExpression<IndexType, Rank, Self, Other<OtherDerived>, SumOp<IndexType>>(*this, dims);
  }
  template <typename OtherDerived>
  inline BinaryDimExpression<IndexType, Rank, Self, Other<OtherDerived>, SubOp<IndexType>> operator-(
      const Other<OtherDerived>& dims) const {
    return BinaryDimExpression<IndexType, Rank, Self, Other<OtherDerived>, SubOp<IndexType>>(*this, dims);
  }
  template <typename OtherDerived>
  inline BinaryDimExpression<IndexType, Rank, Self, Other<OtherDerived>, MulOp<IndexType>> operator*(
      const Other<OtherDerived>& dims) const {
    return BinaryDimExpression<IndexType, Rank, Self, Other<OtherDerived>, MulOp<IndexType>>(*this, dims);
  }
  inline UnaryDimExpression<IndexType, Rank, Self, SumOp<IndexType>> operator+(const IndexType& n) const {
    return UnaryDimExpression<IndexType, Rank, Self, SumOp<IndexType>>(*this, n);
  };
  inline UnaryDimExpression<IndexType, Rank, Self, SubOp<IndexType>> operator-(const IndexType& n) const {
    return UnaryDimExpression<IndexType, Rank, Self, SubOp<IndexType>>(*this, n);
  }
  inline UnaryDimExpression<IndexType, Rank, Self, DivOp<IndexType>> operator/(const IndexType& n) const {
    return UnaryDimExpression<IndexType, Rank, Self, DivOp<IndexType>>(*this, n);
  }
  inline UnaryDimExpression<IndexType, Rank, Self, MulOp<IndexType>> operator*(const IndexType& n) const {
    return UnaryDimExpression<IndexType, Rank, Self, MulOp<IndexType>>(*this, n);
  }
  inline friend UnaryDimExpression<IndexType, Rank, Self, MulOp<IndexType>> operator*(const IndexType& n,
                                                                                      const Self& dims) {
  ...
  inline friend std::ostream& operator<<(std::ostream& os, const Self& dims) { return os << dims.to_string(); }
};
