...
/**
 * An alias for a pair of two tensors of the same rank. It represents
 * observation-objective pairs.
 */
...
class DataProvider {
 ...
 public:
  ...
  /**
   * A method that returns whether the data provider instance has more data to
   * provide. It should always be called before calling get_data(std::size_t).
   *
  ...
  /**
   * Reads and returns the specified number of observation-objective pairs. It
   * also offsets the reader by the specified number. If has_more() returns
   * false, the invocation of this method results in the throwing of a
   * std::out_of_range exception.
   *
   * @param batch_size The maximum number of observation-objective pairs to read
   * and return.
   * @return At most batch_size number of observation-objective pairs. If the
   * number of unread pairs is less than batch_size, the number of returned
   * pairs is that of the unread ones.
   */
