...
/**
 * A utility class for performing gradient checks on neural network and loss
...
class GradientCheck {
 ...
 public:
  /**
   * It performs a gradient check to verify the correctness of the neural
   * network and layer implementations. It is recommended to use double
   * precision floating points.
   *
   * @param provider A reference to the data provider to use for the gradient
   * check.
   * @param net A reference to the network on which the gradient check is to be
   * performed.
   * @param loss The loss function to use for the gradient check.
   * @param verbose Whether the analytic and numerical derivatives of the
   * variables should be printed to the standard out stream.
   ...
   * @param rel_epsilon The maximum acceptable relative (to the greater out of
   * the two) difference between the numerical and analytic gradients.
  ...
  inline static bool verify_gradients(Provider& provider, Net& net, const LossType& loss, bool verbose = true,
    ...
    provider.reset();
    /* As the loss to minimize is the mean of the losses for all the training
     * observations, the gradient to back-propagate is to be divided by the
    ...
    for (auto params_ptr : net.get_all_unique_params()) {
      if (!params_ptr->are_optimizable()) continue;
      Parameters<Scalar>& params = *params_ptr;
      if (verbose) {
        std::cout << "Parameter Set " << std::setw(3) << std::to_string(i) << std::string(28, '-') << std::endl;
      }
      /* Add the derivative of the regularization function w.r.t. to the
       * parameters of the layer to the parameters' gradient. */
      params.regularize();
      ...
      for (int j = 0; j < params_values.rows(); ++j) {
        for (int k = 0; k < params_values.cols(); ++k) {
          if (verbose) std::cout << "\tparam[" << i << "," << j << "," << k << "]:" << std::endl;
          Scalar ana_grad = params_grad(j, k);
          if (verbose) std::cout << "\t\tanalytic gradient = " << ana_grad << std::endl;
          Scalar param = params_values(j, k);
          params_values(j, k) = param + step_size;
          params.set_values(params_values);
          /* Compute the numerical gradients in training mode to ensure that the
           * means and standard deviations used for batch normalization are the
           * same as those used during the analytic gradient computation. */
          Scalar loss_inc = loss.function(net.propagate(data_pair.first, true), data_pair.second).mean();
          /* Calculate the new regularization penalty as its derivative w.r.t.
           * the layer's parameters is included in the gradient. */
          ...
          if (verbose) std::cout << "\t\tnumerical gradient = " << num_grad;
