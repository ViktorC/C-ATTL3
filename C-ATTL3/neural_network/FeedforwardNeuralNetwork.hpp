...
class FeedforwardNeuralNetwork : public NeuralNetwork<Scalar, Rank, false> {
 ...
 public:
  /**
   * @param layers A vector of unique smart pointers to the layers that
   * constitute the neural network.
   * @param foremost Whether the network directly receives its input. If it is
   * set to false, back-propagation returns an empty tensor.
   */
  inline FeedforwardNeuralNetwork(std::vector<LayerPtr<Scalar, Rank>>&& layers, bool foremost = true)
      : layers(std::move(layers)), foremost(foremost) {
  ...
  inline FeedforwardNeuralNetwork(const Self& network)
    ...
    for (std::size_t i = 0; i < layers.size(); ++i) layers[i] = LayerPtr<Scalar, Rank>(network.layers[i]->clone());
  }
  // Move constructor.
  inline FeedforwardNeuralNetwork(Self&& network) { swap(*this, network); }
  ...
  inline Base* clone() const { return new FeedforwardNeuralNetwork(*this); }
  ...
  inline virtual void empty_caches() {
    ...
    for (std::size_t i = 0; i < layers.size(); ++i) layers[i]->empty_cache();
  }
 ...
 private:
  ...
  inline void populate_layer_vector(std::vector<_LayerPtr>& layer_ptrs) const {
    for (std::size_t i = 0; i < layers.size(); ++i) layer_ptrs[i] = layers[i].get();
  }
