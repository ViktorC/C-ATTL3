...
class StackedNeuralNetwork
 ...
 public:
  ...
  inline StackedNeuralNetwork(std::vector<Block>&& blocks, bool foremost = true)
      : blocks(std::move(blocks)), foremost(foremost) {
  ...
  inline StackedNeuralNetwork(const Self& network)
    ...
    for (std::size_t i = 0; i < blocks.size(); ++i) blocks[i] = Block(network.blocks[i]->clone());
  }
  inline StackedNeuralNetwork(Self&& network) { swap(*this, network); }
  ~StackedNeuralNetwork() = default;
  inline Self& operator=(Self network) {
    swap(*this, network);
    return *this;
  }
  inline Base* clone() const { return new StackedNeuralNetwork(*this); }
  ...
  inline std::vector<Base*> get_modules() {
    ...
    for (std::size_t i = 0; i < blocks.size(); ++i) modules.push_back(blocks[i].get());
    return modules;
  }
  ...
  inline void empty_caches() {
    for (std::size_t i = 0; i < blocks.size(); ++i) blocks[i]->empty_caches();
  }
  inline typename Base::Data propagate(typename Base::Data input, bool training) {
    ...
    for (std::size_t i = 0; i < blocks.size(); ++i) input = blocks[i]->propagate(std::move(input), training);
    return input;
  }
  inline typename Base::Data backpropagate(typename Base::Data out_grad) {
    ...
    for (int i = blocks.size() - 1; i >= 0; --i) out_grad = blocks[i]->backpropagate(std::move(out_grad));
    return out_grad;
  }
