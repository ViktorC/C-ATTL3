...
class UnidirectionalNeuralNetwork : public NeuralNetwork<Scalar, Rank, true> {
 ...
 public:
  ...
  /**
   * @return Whether the direction along the time-step rank in which the network
   * processes its inputs is reversed.
   */
  virtual bool is_reversed() const;
  /**
   * Flips the direction along the time-step rank in which the network processes
