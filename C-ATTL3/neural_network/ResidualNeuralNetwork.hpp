...
/**
 * A class template for ResNets. These networks take vectors of neural networks
 * as their sub-modules.
 *
 * \see https://arxiv.org/abs/1512.03385
 */
template <typename Scalar, std::size_t Rank>
class ResidualNeuralNetwork : public CompositeNeuralNetwork<Scalar, Rank, false, NeuralNetwork<Scalar, Rank, false>> {
 ...
 public:
  ...
  inline ResidualNeuralNetwork(std::vector<Module>&& modules, bool foremost = true)
    ...
    for (std::size_t i = 0; i < this->modules.size(); ++i) {
      ...
      if (i != 0) module.set_foremost(false);
  ...
  inline ResidualNeuralNetwork(Self&& network) { swap(*this, network); }
  ~ResidualNeuralNetwork() = default;
  inline Self& operator=(Self network) {
    swap(*this, network);
    return *this;
  }
  inline Base* clone() const { return new ResidualNeuralNetwork(*this); }
  ...
  inline typename Base::Data propagate(typename Base::Data input, bool training) {
    ...
    for (std::size_t i = 0; i < modules.size(); ++i) input += modules[i]->propagate(input, training);
    return input;
  }
