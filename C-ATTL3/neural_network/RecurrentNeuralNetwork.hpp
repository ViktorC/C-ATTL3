...
/**
 * A class template for a simple recurrent neural network (RNN). The network can
 * use multiplicative integration to combine its linearly transformed input and
 * its linearly transformed previous hidden state. A stateful network retains
 * its hidden state across sequences as long as the batch size is constant.
 */
...
class RecurrentNeuralNetwork : public UnidirectionalNeuralNetwork<Scalar, Rank> {
 ...
 public:
  /**
   * @param input_kernel The linear layer applied to the input of the network at
   * each time step with an input.
   * @param state_kernel The linear layer applied to the previous hidden state
   * of the network at each time step.
   * @param output_kernel The linear layer applied to the hidden state of the
   * network at each time step with an output.
   * @param state_act The activation function applied to the hidden state at
   * each time step.
   * @param output_act The activation function applied to the linearly
   * transformed hidden state of the network at each time step with an output.
  ...
  inline RecurrentNeuralNetwork(Self&& network) { swap(*this, network); }
  ~RecurrentNeuralNetwork() = default;
  inline Self& operator=(Self network) {
    swap(*this, network);
    return *this;
  }
  inline Root* clone() const { return new RecurrentNeuralNetwork(*this); }
  ...
  inline typename Root::Data propagate(typename Root::Data input, bool training) {
    ...
    // Calculate the output sequence length and delay based on the input
    // sequence length.
    ...
    // If in training mode, unroll the network (unless it has already been
    // unrolled for the same alignment).
    ...
    // If the output is a single time step prediction, there is no need to
    ...
    for (int i = 0; i < time_steps; ++i) {
      ...
      if (i < input_seq_length) {
        ...
        if (MulInt) {
          if (training) {
            /* If multiplicative integration is enabled, cache the factors of
      ...
      if (i >= output_seq_delay && i < output_end) {
        ...
        TimeStepData act_out_i =
            cell.output_act->pass_forward(cell.output_kernel->pass_forward(state, training), training);
  ...
  inline typename Root::Data backpropagate(typename Root::Data out_grad) {
    ...
    for (int i = time_steps - 1; i >= 0; --i) {
      ...
      if (i >= output_seq_delay && i < output_end) {
        ...
        state_grad += cell.output_kernel->pass_back(
            cell.output_act->pass_back(TensorMap<Scalar, Rank + 1>(out_grad_seq_i.data(), out_time_step_dims)));
      }
      ...
      if (i < input_seq_length) {
        // If it is the foremost layer, the gradients do not need to be
 ...
 private:
  ...
  inline void unroll_network(std::size_t time_steps, std::size_t input_seq_length, std::size_t output_seq_delay,
                             std::size_t output_end) {
    if (time_steps > 1) {
      // Empty the caches of the main cell to reduce the amount of data to copy.
      empty_caches();
      // Emptying the caches also clears the cell vector, thus it has to be
      ...
      for (int j = 1; j < time_steps; ++j) {
        ...
        cell.state_kernel = KernelPtr<Scalar, Rank>(
            static_cast<KernelLayer<Scalar, Rank>*>(main_cell.state_kernel->clone_with_shared_params()));
        ...
        if (j < input_seq_length) {
          cell.input_kernel = KernelPtr<Scalar, Rank>(
              static_cast<KernelLayer<Scalar, Rank>*>(main_cell.input_kernel->clone_with_shared_params()));
        }
        if (j >= output_seq_delay && j < output_end) {
          cell.output_kernel = KernelPtr<Scalar, Rank>(
              static_cast<KernelLayer<Scalar, Rank>*>(main_cell.output_kernel->clone_with_shared_params()));
          cell.output_act = ActivationPtr<Scalar, Rank>(
              static_cast<ActivationLayer<Scalar, Rank>*>(main_cell.output_act->clone_with_shared_params()));
        }
      }
    } else
      cells = std::vector<Cell>(0);
  ...
  struct Cell {
    ...
    inline Cell(const Cell& cell)
        : input_kernel(KernelPtr<Scalar, Rank>(static_cast<KernelLayer<Scalar, Rank>*>(cell.input_kernel->clone()))),
          state_kernel(KernelPtr<Scalar, Rank>(static_cast<KernelLayer<Scalar, Rank>*>(cell.state_kernel->clone()))),
          output_kernel(KernelPtr<Scalar, Rank>(static_cast<KernelLayer<Scalar, Rank>*>(cell.output_kernel->clone()))),
          state_act(ActivationPtr<Scalar, Rank>(static_cast<ActivationLayer<Scalar, Rank>*>(cell.state_act->clone()))),
          output_act(
              ActivationPtr<Scalar, Rank>(static_cast<ActivationLayer<Scalar, Rank>*>(cell.output_act->clone()))),
