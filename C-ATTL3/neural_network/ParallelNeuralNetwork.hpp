 ...
 * lanes of non-sequential neural networks with the same input dimensions.
 * Inputs and gradients are propagated through the lanes simultaneously using
 * multithreading. The outputs of the lanes are merged by concatenation (either
 * along the lowest or hightest rank), summation, or multiplication.
 *
...
class ParallelNeuralNetwork : public CompositeNeuralNetwork<Scalar, Rank, false, NeuralNetwork<Scalar, Rank, false>> {
 ...
 public:
  ...
  inline ParallelNeuralNetwork(std::vector<Lane>&& lanes, bool foremost = true)
      : lanes(std::move(lanes)), foremost(foremost), outputs(this->lanes.size()) {
    ...
    for (std::size_t i = 1; i < this->lanes.size(); ++i) {
      ...
      if (MergeType == PARALLEL_CONCAT_HI_RANK || MergeType == PARALLEL_CONCAT_LO_RANK) {
        if (MergeType == PARALLEL_CONCAT_HI_RANK) {
          for (std::size_t i = 0; i < +CONCAT_RANK; ++i) assert(output_dims(i) == lane_output_dims(i));
        } else {
          for (std::size_t i = Rank - 1; i > +CONCAT_RANK; --i) assert(output_dims(i) == lane_output_dims(i));
        }
  ...
  inline ParallelNeuralNetwork(const Self& network)
    ...
    for (std::size_t i = 0; i < lanes.size(); ++i) lanes[i] = Lane(network.lanes[i]->clone());
  }
  inline ParallelNeuralNetwork(Self&& network) { swap(*this, network); }
  ~ParallelNeuralNetwork() = default;
  inline Self& operator=(Self network) {
    swap(*this, network);
    return *this;
  }
  inline Base* clone() const { return new ParallelNeuralNetwork(*this); }
  ...
  inline std::vector<Base*> get_modules() {
    ...
    for (std::size_t i = 0; i < lanes.size(); ++i) modules.push_back(lanes[i].get());
    return modules;
  }
  inline bool is_foremost() const { return foremost; }
  inline void set_foremost(bool foremost) {
    for (std::size_t i = 0; i < lanes.size(); ++i) lanes[i]->set_foremost(foremost);
  ...
  inline typename Base::Data propagate(typename Base::Data input, bool training) {
    ...
    for (std::size_t i = 0; i < lane_num; ++i) {
      if (i == 0) {
        ...
        if (MergeType == PARALLEL_MUL) outputs[i] = out;
      } else {
  ...
  inline typename Base::Data backpropagate(typename Base::Data out_grad) {
    ...
    for (std::size_t i = 0; i < lanes.size(); ++i) {
      ...
      if (!foremost) prev_out_grad += args_arr[i].prev_out_grad;
    }
    if (helper_thread_num > 0) {
      pthread_state = pthread_attr_destroy(&attr);
      assert(!pthread_state);
  ...
  inline static void* backpropagate(void* args_ptr) {
    ...
    if (MergeType == PARALLEL_SUM)
      ...
      for (std::size_t i = 0; i < args.obj->lanes.size(); ++i) {
        if (i != (std::size_t)args.lane_id) out_grad *= args.obj->outputs[i];
      }
