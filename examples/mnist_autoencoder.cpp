...
int main() {
  ...
  MNISTDataProvider<float> file_train_prov(mnist_folder + "train-images.idx3-ubyte",
                                           mnist_folder + "train-labels.idx1-ubyte");
  MNISTDataProvider<float> file_test_prov(mnist_folder + "t10k-images.idx3-ubyte",
  ...
  encoder_layers[0] =
      LayerPtr<float, 3>(new ConvKernelLayer<float>(train_prov.get_obs_dims(), 3, init, 4, 4, 0, 0, 2, 2));
  encoder_layers[1] = LayerPtr<float, 3>(new SoftplusActivationLayer<float, 3>(encoder_layers[0]->get_output_dims()));
  encoder_layers[2] =
      LayerPtr<float, 3>(new ConvKernelLayer<float>(encoder_layers[1]->get_output_dims(), 3, init, 4, 4, 0, 0, 1, 1));
  encoder_layers[3] = LayerPtr<float, 3>(new SoftplusActivationLayer<float, 3>(encoder_layers[2]->get_output_dims()));
  encoder_layers[4] =
      LayerPtr<float, 3>(new DenseKernelLayer<float, 3>(encoder_layers[2]->get_output_dims(), 100, init));
  ...
  decoder_layers[2] =
      LayerPtr<float, 3>(new ReshapeLayer<float, 3>(decoder_layers[1]->get_output_dims(), {10u, 10u, 3u}));
  decoder_layers[3] = LayerPtr<float, 3>(
      new TransConvKernelLayer<float>(decoder_layers[2]->get_output_dims(), 3, init, 4, 4, 0, 0, 1, 1));
  decoder_layers[4] = LayerPtr<float, 3>(new SoftplusActivationLayer<float, 3>(decoder_layers[3]->get_output_dims()));
  decoder_layers[5] = LayerPtr<float, 3>(
      new TransConvKernelLayer<float>(decoder_layers[4]->get_output_dims(), 1, init, 4, 4, 0, 0, 2, 2));
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    ppm_codec.encode(TensorMap<float, 3>(image.data(), {28u, 28u, 1u}),
                     std::string("image") + std::to_string(i) + std::string(".ppm"));
    ...
    ppm_codec.encode(TensorMap<float, 3>(decoded_image.data(), {28u, 28u, 1u}),
                     std::string("image") + std::to_string(i) + std::string("_decoded.ppm"));
  }
}
