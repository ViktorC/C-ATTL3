...
int main() {
  ...
  MNISTDataProvider<float> real_file_train_prov(mnist_folder + "train-images.idx3-ubyte",
  ...
  MNISTDataProvider<float> real_file_test_prov(mnist_folder + "t10k-images.idx3-ubyte",
  ...
  generator_layers[1] =
      LayerPtr<float, 3>(new LeakyReLUActivationLayer<float, 3>(generator_layers[0]->get_output_dims(), lrelu_alpha));
  generator_layers[2] =
      LayerPtr<float, 3>(new DenseKernelLayer<float, 3>(generator_layers[1]->get_output_dims(), 512, init));
  generator_layers[3] =
      LayerPtr<float, 3>(new LeakyReLUActivationLayer<float, 3>(generator_layers[2]->get_output_dims(), lrelu_alpha));
  generator_layers[4] =
      LayerPtr<float, 3>(new DenseKernelLayer<float, 3>(generator_layers[3]->get_output_dims(), 1024, init));
  generator_layers[5] =
      LayerPtr<float, 3>(new LeakyReLUActivationLayer<float, 3>(generator_layers[4]->get_output_dims(), lrelu_alpha));
  generator_layers[6] =
      LayerPtr<float, 3>(new DenseKernelLayer<float, 3>(generator_layers[5]->get_output_dims(), 784, init));
  generator_layers[7] = LayerPtr<float, 3>(new TanhActivationLayer<float, 3>(generator_layers[6]->get_output_dims()));
  generator_layers[8] =
      LayerPtr<float, 3>(new ReshapeLayer<float, 3>(generator_layers[7]->get_output_dims(), {28u, 28u, 1u}));
  ...
  discriminator_layers[0] =
      LayerPtr<float, 3>(new DenseKernelLayer<float, 3>(generator->get_output_dims(), 1024, init));
  discriminator_layers[1] = LayerPtr<float, 3>(
      new LeakyReLUActivationLayer<float, 3>(discriminator_layers[0]->get_output_dims(), lrelu_alpha));
  discriminator_layers[2] =
      LayerPtr<float, 3>(new DropoutLayer<float, 3>(discriminator_layers[1]->get_output_dims(), dropout));
  discriminator_layers[3] =
      LayerPtr<float, 3>(new DenseKernelLayer<float, 3>(discriminator_layers[2]->get_output_dims(), 512, init));
  discriminator_layers[4] = LayerPtr<float, 3>(
      new LeakyReLUActivationLayer<float, 3>(discriminator_layers[3]->get_output_dims(), lrelu_alpha));
  discriminator_layers[5] =
      LayerPtr<float, 3>(new DropoutLayer<float, 3>(discriminator_layers[4]->get_output_dims(), dropout));
  discriminator_layers[6] =
      LayerPtr<float, 3>(new DenseKernelLayer<float, 3>(discriminator_layers[5]->get_output_dims(), 256, init));
  discriminator_layers[7] = LayerPtr<float, 3>(
      new LeakyReLUActivationLayer<float, 3>(discriminator_layers[6]->get_output_dims(), lrelu_alpha));
  discriminator_layers[8] =
      LayerPtr<float, 3>(new DropoutLayer<float, 3>(discriminator_layers[7]->get_output_dims(), dropout));
  discriminator_layers[9] =
      LayerPtr<float, 3>(new DenseKernelLayer<float, 3>(discriminator_layers[8]->get_output_dims(), 1, init));
  discriminator_layers[10] =
      LayerPtr<float, 3>(new SigmoidActivationLayer<float, 3>(discriminator_layers[9]->get_output_dims()));
  ...
  for (unsigned i = 0; i <= epochs; ++i) {
    ...
    while (real_train_prov.has_more()) {
      ...
      total_m += actual_m;
      Tensor<float, 4> train_data = gan.get_modules()[0]
                                        ->infer(Tensor<float, 4>(actual_m, 1u, 1u, 100u).random())
                                        .concatenate(std::move(real_train_data.first), 0);
      Tensor<float, 4> train_label =
          Tensor<float, 4>(actual_m, 1u, 1u, 1u).constant(0).concatenate(std::move(real_train_data.second), 0);
    ...
    Tensor<float, 4> test_data = gan.get_modules()[0]
                                     ->infer(Tensor<float, 4>(actual_m, 1u, 1u, 100u).random())
                                     .concatenate(std::move(real_test_data.first), 0);
    Tensor<float, 4> test_label =
        Tensor<float, 4>(actual_m, 1u, 1u, 1u).constant(0).concatenate(std::move(real_test_data.second), 0);
    ...
    for (unsigned j = 0; j < 10; ++j) {
      ...
      ppm_codec.encode(
          TensorMap<float, 3>(fake_image_sample.data(), {28u, 28u, 1u}),
          std::string("image") + std::to_string(i) + std::string("_") + std::to_string(j) + std::string(".pgm"));
    }
  }
}
