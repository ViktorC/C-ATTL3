...
int main() {
	...
	auto reg = std::make_shared<NoParameterRegularization<float>>();
	...
	generator_layers[1] = LayerPtr<float,3>(new LeakyReLUActivationLayer<float,3>(generator_layers[0]->get_output_dims()));
	...
	generator_layers[3] = LayerPtr<float,3>(new LeakyReLUActivationLayer<float,3>(generator_layers[2]->get_output_dims()));
	...
	generator_layers[5] = LayerPtr<float,3>(new LeakyReLUActivationLayer<float,3>(generator_layers[4]->get_output_dims()));
	...
	generator_layers[7] = LayerPtr<float,3>(new LeakyReLUActivationLayer<float,3>(generator_layers[6]->get_output_dims()));
	...
	discriminator_layers[1] = LayerPtr<float,3>(new LeakyReLUActivationLayer<float,3>(discriminator_layers[0]->get_output_dims()));
	discriminator_layers[2] = LayerPtr<float,3>(new BatchNormLayer<float,3>(discriminator_layers[1]->get_output_dims()));
	discriminator_layers[3] = LayerPtr<float,3>(new ConvolutionLayer<float>(discriminator_layers[2]->get_output_dims(), 16, init, reg, 4, 4, 2, 2, 2, 2));
	discriminator_layers[4] = LayerPtr<float,3>(new LeakyReLUActivationLayer<float,3>(discriminator_layers[3]->get_output_dims()));
	discriminator_layers[5] = LayerPtr<float,3>(new BatchNormLayer<float,3>(discriminator_layers[4]->get_output_dims()));
	discriminator_layers[6] = LayerPtr<float,3>(new ConvolutionLayer<float>(discriminator_layers[5]->get_output_dims(), 32, init, reg, 4, 4, 1, 1, 2, 2));
	discriminator_layers[7] = LayerPtr<float,3>(new LeakyReLUActivationLayer<float,3>(discriminator_layers[6]->get_output_dims()));
	discriminator_layers[8] = LayerPtr<float,3>(new BatchNormLayer<float,3>(discriminator_layers[7]->get_output_dims()));
	discriminator_layers[9] = LayerPtr<float,3>(new ConvolutionLayer<float>(discriminator_layers[8]->get_output_dims(), 64, init, reg, 4, 4, 0, 0));
	discriminator_layers[10] = LayerPtr<float,3>(new LeakyReLUActivationLayer<float,3>(discriminator_layers[9]->get_output_dims()));
	...
	NadamOptimizer<float,3,false> disc_opt(loss, m, 1e-3, .5, 5e-3);
	NadamOptimizer<float,3,false> gen_opt(neg_loss, m, 1e-3, .5, 5e-3);
	...
	for (unsigned i = 0; i <= epochs; ++i) {
		...
		for (unsigned j = 0; j < 10; ++j) {
			auto fake_image_sample = gan.get_modules()[0]->infer(Tensor<float,4>(1u, 1u, 1u, 100u).template random<NormalRandGen<float>>());
