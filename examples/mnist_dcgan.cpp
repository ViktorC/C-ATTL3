...
int main() {
	...
	generator_layers[4] = LayerPtr<float,3>(new LeakyReLUActivationLayer<float,3>(generator_layers[3]->get_output_dims()));
	...
	generator_layers[10] = LayerPtr<float,3>(new LeakyReLUActivationLayer<float,3>(generator_layers[9]->get_output_dims()));
	...
	AdamOptimizer<float,3,false> disc_opt(loss, m);
	AdamOptimizer<float,3,false> gen_opt(neg_loss, m);
