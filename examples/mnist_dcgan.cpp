...
int main() {
	...
	MemoryDataProvider<float,3,false> real_train_prov(
			...
			TensorPtr<float,4>(new Tensor<float,4>(Tensor<float,4>(60000u, 1u, 1u, 1u).constant(1))));
	...
	MemoryDataProvider<float,3,false> real_test_prov(
			...
			TensorPtr<float,4>(new Tensor<float,4>(Tensor<float,4>(10000u, 1u, 1u, 1u).constant(1))));
	// Create the GAN.
	auto init = std::make_shared<HeWeightInitialization<float>>(1e-2);
	auto reg = std::make_shared<SquaredParameterRegularization<float>>();
	std::vector<LayerPtr<float,3>> generator_layers(10);
	generator_layers[0] = LayerPtr<float,3>(new DeconvolutionLayer<float>({ 1u, 1u, 100u }, 64, init, reg, 4, 4, 0, 0));
	generator_layers[1] = LayerPtr<float,3>(new TanhActivationLayer<float,3>(generator_layers[0]->get_output_dims()));
	generator_layers[2] = LayerPtr<float,3>(new DeconvolutionLayer<float>(generator_layers[1]->get_output_dims(), 32, init, reg, 4, 4, 1, 1, 2, 2));
	generator_layers[3] = LayerPtr<float,3>(new TanhActivationLayer<float,3>(generator_layers[2]->get_output_dims()));
	generator_layers[4] = LayerPtr<float,3>(new DeconvolutionLayer<float>(generator_layers[3]->get_output_dims(), 16, init, reg, 4, 4, 2, 2, 2, 2));
	generator_layers[5] = LayerPtr<float,3>(new TanhActivationLayer<float,3>(generator_layers[4]->get_output_dims()));
	generator_layers[6] = LayerPtr<float,3>(new DeconvolutionLayer<float>(generator_layers[5]->get_output_dims(), 8, init, reg, 4, 4, 1, 1, 2, 2));
	generator_layers[7] = LayerPtr<float,3>(new TanhActivationLayer<float,3>(generator_layers[6]->get_output_dims()));
	generator_layers[8] = LayerPtr<float,3>(new DeconvolutionLayer<float>(generator_layers[7]->get_output_dims(), 1, init));
	generator_layers[9] = LayerPtr<float,3>(new TanhActivationLayer<float,3>(generator_layers[8]->get_output_dims()));
	NeuralNetPtr<float,3,false> generator(new FeedforwardNeuralNetwork<float,3>(std::move(generator_layers)));
	std::vector<LayerPtr<float,3>> discriminator_layers(10);
	discriminator_layers[0] = LayerPtr<float,3>(new ConvolutionLayer<float>(generator->get_output_dims(), 8, init, reg, 4, 4, 1, 1, 2, 2));
	discriminator_layers[1] = LayerPtr<float,3>(new TanhActivationLayer<float,3>(discriminator_layers[0]->get_output_dims()));
	discriminator_layers[2] = LayerPtr<float,3>(new ConvolutionLayer<float>(discriminator_layers[1]->get_output_dims(), 16, init, reg, 4, 4, 2, 2, 2, 2));
	discriminator_layers[3] = LayerPtr<float,3>(new TanhActivationLayer<float,3>(discriminator_layers[2]->get_output_dims()));
	discriminator_layers[4] = LayerPtr<float,3>(new ConvolutionLayer<float>(discriminator_layers[3]->get_output_dims(), 32, init, reg, 4, 4, 1, 1, 2, 2));
	discriminator_layers[5] = LayerPtr<float,3>(new TanhActivationLayer<float,3>(discriminator_layers[4]->get_output_dims()));
	discriminator_layers[6] = LayerPtr<float,3>(new ConvolutionLayer<float>(discriminator_layers[5]->get_output_dims(), 64, init, reg, 4, 4, 0, 0));
	discriminator_layers[7] = LayerPtr<float,3>(new TanhActivationLayer<float,3>(discriminator_layers[6]->get_output_dims()));
	discriminator_layers[8] = LayerPtr<float,3>(new DenseLayer<float,3>(discriminator_layers[7]->get_output_dims(), 1, init));
	discriminator_layers[9] = LayerPtr<float,3>(new SigmoidActivationLayer<float,3>(discriminator_layers[8]->get_output_dims()));
	...
	AdamOptimizer<float,3,false> gen_opt(loss, m, 2e-4, .5);
	...
	for (unsigned i = 0; i <= epochs; ++i) {
		...
		while (real_train_prov.has_more()) {
			...
			Tensor<float,4> train_label = Tensor<float,4>(actual_m, 1u, 1u, 1u).constant(0)
					.concatenate(std::move(real_train_data.second), 0);
			...
			// Then optimize the generator by minimizing the GAN's loss on classifying generated images labelled as real.
			...
			// Freeze the discriminator's parameters when optimizing the generator.
		...
		Tensor<float,4> test_label = Tensor<float,4>(actual_m, 1u, 1u, 1u).constant(0)
				.concatenate(std::move(real_test_data.second), 0);
		...
		for (unsigned j = 0; j < 10; ++j) {
			Tensor<float,4> fake_image_sample = gan.get_modules()[0]->infer(Tensor<float,4>(1u, 1u, 1u, 100u).random());
