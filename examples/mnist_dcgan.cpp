...
int main() {
	...
	generator_layers[0] = LayerPtr<float,3>(new DeconvolutionLayer<float>({ 1u, 1u, 100u }, 1024, init, reg, 4, 4, 0, 0));
	generator_layers[1] = LayerPtr<float,3>(new BatchNormLayer<float,3>(generator_layers[0]->get_output_dims()));
	generator_layers[2] = LayerPtr<float,3>(new LeakyReLUActivationLayer<float,3>(generator_layers[1]->get_output_dims(), .2));
	generator_layers[3] = LayerPtr<float,3>(new DeconvolutionLayer<float>(generator_layers[2]->get_output_dims(), 512, init, reg, 4, 4, 1, 1, 2, 2));
	generator_layers[4] = LayerPtr<float,3>(new BatchNormLayer<float,3>(generator_layers[3]->get_output_dims()));
	generator_layers[5] = LayerPtr<float,3>(new LeakyReLUActivationLayer<float,3>(generator_layers[4]->get_output_dims(), .2));
	generator_layers[6] = LayerPtr<float,3>(new DeconvolutionLayer<float>(generator_layers[5]->get_output_dims(), 256, init, reg, 4, 4, 2, 2, 2, 2));
	generator_layers[7] = LayerPtr<float,3>(new BatchNormLayer<float,3>(generator_layers[6]->get_output_dims()));
	generator_layers[8] = LayerPtr<float,3>(new LeakyReLUActivationLayer<float,3>(generator_layers[7]->get_output_dims(), .2));
	generator_layers[9] = LayerPtr<float,3>(new DeconvolutionLayer<float>(generator_layers[8]->get_output_dims(), 128, init, reg, 4, 4, 1, 1, 2, 2));
	generator_layers[10] = LayerPtr<float,3>(new BatchNormLayer<float,3>(generator_layers[9]->get_output_dims()));
	generator_layers[11] = LayerPtr<float,3>(new LeakyReLUActivationLayer<float,3>(generator_layers[10]->get_output_dims(), .2));
	...
	discriminator_layers[0] = LayerPtr<float,3>(new ConvolutionLayer<float>(generator->get_output_dims(), 128, init, reg, 4, 4, 1, 1, 2, 2));
	discriminator_layers[1] = LayerPtr<float,3>(new LeakyReLUActivationLayer<float,3>(discriminator_layers[0]->get_output_dims(), .2));
	discriminator_layers[2] = LayerPtr<float,3>(new BatchNormLayer<float,3>(discriminator_layers[1]->get_output_dims()));
	discriminator_layers[3] = LayerPtr<float,3>(new ConvolutionLayer<float>(discriminator_layers[2]->get_output_dims(), 256, init, reg, 4, 4, 2, 2, 2, 2));
	discriminator_layers[4] = LayerPtr<float,3>(new LeakyReLUActivationLayer<float,3>(discriminator_layers[3]->get_output_dims(), .2));
	discriminator_layers[5] = LayerPtr<float,3>(new BatchNormLayer<float,3>(discriminator_layers[4]->get_output_dims()));
	discriminator_layers[6] = LayerPtr<float,3>(new ConvolutionLayer<float>(discriminator_layers[5]->get_output_dims(), 512, init, reg, 4, 4, 1, 1, 2, 2));
	discriminator_layers[7] = LayerPtr<float,3>(new LeakyReLUActivationLayer<float,3>(discriminator_layers[6]->get_output_dims(), .2));
	discriminator_layers[8] = LayerPtr<float,3>(new BatchNormLayer<float,3>(discriminator_layers[7]->get_output_dims()));
	discriminator_layers[9] = LayerPtr<float,3>(new ConvolutionLayer<float>(discriminator_layers[8]->get_output_dims(), 1024, init, reg, 4, 4, 0, 0));
	discriminator_layers[10] = LayerPtr<float,3>(new LeakyReLUActivationLayer<float,3>(discriminator_layers[9]->get_output_dims(), .2));
