...
//	ResidualNeuralNetwork<Scalar> nn(res_modules);
	std::vector<NeuralNetPtr<Scalar>> modules;
	std::vector<CompositeNeuralNetwork<Scalar>> sub_modules;
	sub_modules.push_back(CompositeNeuralNetwork<Scalar>(NeuralNetPtr<Scalar>(new SequentialNeuralNetwork<Scalar>(
			LayerPtr<Scalar>(new ConvLayer<Scalar>(training_prov.get_obs_dims(), 5, init))))));
	sub_modules.push_back(CompositeNeuralNetwork<Scalar>(NeuralNetPtr<Scalar>(new SequentialNeuralNetwork<Scalar>(
			LayerPtr<Scalar>(new LeakyReLUActivationLayer<Scalar>(sub_modules[0].get_output_dims().add_along_depth(sub_modules[0].get_input_dims())))))));
	sub_modules.push_back(CompositeNeuralNetwork<Scalar>(NeuralNetPtr<Scalar>(new SequentialNeuralNetwork<Scalar>(
			LayerPtr<Scalar>(new BatchNormLayer<Scalar>(sub_modules[1].get_output_dims().add_along_depth(sub_modules[1].get_input_dims())))))));
	modules.push_back(NeuralNetPtr<Scalar>(new DenseNeuralNetwork<Scalar>(sub_modules)));
	modules.push_back(NeuralNetPtr<Scalar>(new SequentialNeuralNetwork<Scalar>(
			LayerPtr<Scalar>(new FCLayer<Scalar>(modules[0]->get_output_dims(), 1, init)))));
	CompositeNeuralNetwork<Scalar> nn(std::move(modules));
	nn.init();
