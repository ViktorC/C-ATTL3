...
int main() {
	...
	PCAPreprocessor<Scalar,RANK> preproc;
	TensorPtr<Scalar,RANK + 1> training_obs_ptr = TensorPtr<Scalar,RANK + 1>(new Tensor<Scalar,RANK + 1>(80, 32, 32));
	TensorPtr<Scalar,RANK + 1> training_obj_ptr = TensorPtr<Scalar,RANK + 1>(new Tensor<Scalar,RANK + 1>(80, 1, 1));
	...
	TensorPtr<Scalar,RANK + 1> test_obs_ptr = TensorPtr<Scalar,RANK + 1>(new Tensor<Scalar,RANK + 1>(20, 32, 32));
	TensorPtr<Scalar,RANK + 1> test_obj_ptr = TensorPtr<Scalar,RANK + 1>(new Tensor<Scalar,RANK + 1>(20, 1, 1));
...
//	ResidualNeuralNetwork<Scalar,RANK> nn(res_modules);
	std::vector<LayerPtr<Scalar,RANK>> layers(3);
	layers[0] = LayerPtr<Scalar,RANK>(new FCLayer<Scalar,RANK>(training_prov.get_obs_dims(), 50, init));
	layers[1] = LayerPtr<Scalar,RANK>(new ReLUActivationLayer<Scalar,RANK>(layers[0]->get_output_dims()));
	layers[2] = LayerPtr<Scalar,RANK>(new FCLayer<Scalar,RANK>(layers[1]->get_output_dims(), 1, init));
	SequentialNeuralNetwork<Scalar,RANK> nn(std::move(layers));
	nn.init();
