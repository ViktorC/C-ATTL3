...
int main() {
	std::string mnist_folder = "C:\\Users\\Viktor\\Downloads\\mnist\\";
	MNISTDataProvider<fp> file_train_prov(mnist_folder + "train-images.idx3-ubyte", mnist_folder + "train-labels.idx1-ubyte");
	MNISTDataProvider<fp> file_test_prov(mnist_folder + "t10k-images.idx3-ubyte", mnist_folder + "t10k-labels.idx1-ubyte");
	DataPair<fp,3,false> train_data = file_train_prov.get_data(60000);
	TensorPtr<fp,4> train_obs(new Tensor<fp,4>(std::move(train_data.first)));
	TensorPtr<fp,4> train_obj(new Tensor<fp,4>(std::move(train_data.second)));
	MemoryDataProvider<fp,3,false> train_prov(std::move(train_obs), std::move(train_obj));
	DataPair<fp,3,false> test_data = file_test_prov.get_data(1);
	TensorPtr<fp,4> test_obs(new Tensor<fp,4>(std::move(test_data.first)));
	TensorPtr<fp,4> test_obj(new Tensor<fp,4>(std::move(test_data.second)));
	MemoryDataProvider<fp,3,false> test_prov(std::move(test_obs), std::move(test_obj));
	auto init = std::make_shared<GlorotWeightInitialization<fp>>();
	std::vector<LayerPtr<fp,3>> layers(9);
	layers[0] = LayerPtr<fp,3>(new ConvLayer<fp>(train_prov.get_obs_dims(), 8, init, ConvLayer<fp>::NO_PARAM_REG, 5, 5, 2, 2));
	layers[1] = LayerPtr<fp,3>(new SigmoidActivationLayer<fp,3>(layers[0]->get_output_dims()));
	layers[2] = LayerPtr<fp,3>(new MaxPoolingLayer<fp>(layers[1]->get_output_dims()));
	layers[3] = LayerPtr<fp,3>(new ConvLayer<fp>(layers[2]->get_output_dims(), 8, init, ConvLayer<fp>::NO_PARAM_REG, 5, 5, 2, 2));
	layers[4] = LayerPtr<fp,3>(new SigmoidActivationLayer<fp,3>(layers[3]->get_output_dims()));
	layers[5] = LayerPtr<fp,3>(new MaxPoolingLayer<fp>(layers[4]->get_output_dims()));
	layers[6] = LayerPtr<fp,3>(new FCLayer<fp,3>(layers[5]->get_output_dims(), 128, init));
	layers[7] = LayerPtr<fp,3>(new SigmoidActivationLayer<fp,3>(layers[6]->get_output_dims()));
	layers[8] = LayerPtr<fp,3>(new FCLayer<fp,3>(layers[7]->get_output_dims(), 10, init));
//	layers[0] = LayerPtr<fp,3>(new FCLayer<fp,3>(train_prov.get_obs_dims(), 500, init));
//	layers[1] = LayerPtr<fp,3>(new SigmoidActivationLayer<fp,3>(layers[0]->get_output_dims()));
//	layers[2] = LayerPtr<fp,3>(new FCLayer<fp,3>(layers[1]->get_output_dims(), 250, init));
//	layers[3] = LayerPtr<fp,3>(new SigmoidActivationLayer<fp,3>(layers[2]->get_output_dims()));
//	layers[4] = LayerPtr<fp,3>(new FCLayer<fp,3>(layers[3]->get_output_dims(), 10, init));
	FeedforwardNeuralNetwork<fp,3> nn(std::move(layers));
	nn.init();
	auto loss = std::make_shared<SoftmaxCrossEntropyLoss<fp,3,false>>();
	VanillaSGDOptimizer<fp,3,false> opt(loss, 100);
	std::chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();
