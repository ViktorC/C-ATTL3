...
int main() {
	...
	layers1[3] = LayerPtr<Scalar,RANK>(new ConvLayer<Scalar,RANK>(layers1[2]->get_output_dims(), 8, init));					// 5
	layers1[4] = LayerPtr<Scalar,RANK>(new MaxPoolingLayer<Scalar,RANK>(layers1[3]->get_output_dims()));					// 6
	...
	layers2[0] = LayerPtr<Scalar,RANK>(new ConvLayer<Scalar,RANK>(res_modules[0].first.get_output_dims(), 8, init));		// 9
	...
	res_modules.push_back(std::make_pair(CompositeNeuralNetwork<Scalar,RANK>(NeuralNetPtr<Scalar,RANK>(
			new SequentialNeuralNetwork<Scalar,RANK>(LayerPtr<Scalar,RANK>(new MaxPoolingLayer<Scalar,RANK>(
					res_modules[1].first.get_output_dims()))))), false));													// 12
	std::vector<LayerPtr<Scalar,RANK>> layers3(3);
	layers3[0] = LayerPtr<Scalar,RANK>(new ConvLayer<Scalar,RANK>(res_modules[2].first.get_output_dims(), 8, init));		// 13
	...
	layers4[0] = LayerPtr<Scalar,RANK>(new MaxPoolingLayer<Scalar,RANK>(res_modules[3].first.get_output_dims()));			// 16
