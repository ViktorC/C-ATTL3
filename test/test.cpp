...
//	preproc.transform(data);
	...
	layers[2] = new cppnn::ConvLayer<Scalar>(layers[1]->get_output_dims(), init, 5, 5, 3, 3);
	layers[3] = new cppnn::LeakyReLUActivationLayer<Scalar>(layers[2]->get_output_dims());
	layers[4] = new cppnn::BatchNormLayer<Scalar>(layers[3]->get_output_dims());
	layers[5] = new cppnn::DenseLayer<Scalar>(layers[4]->get_output_dims(), 1, init);
