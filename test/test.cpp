...
//	RegPenSharedPtr<float> reg_pen(new ElasticNetRegularizationPenalty<float>(1e-4, 1e-4));
//	std::vector<LayerPtr<float,3>> layers(9);
//	layers[0] = LayerPtr<float,3>(new ConvLayer<float>(training_prov.get_obs_dims(), 8, init, reg_pen, 5, 5, 2));
...
//	layers[3] = LayerPtr<float,3>(new ConvLayer<float>(layers[2]->get_output_dims(), 8, init, reg_pen, 5, 5, 2));
...
//	layers[6] = LayerPtr<float,3>(new FCLayer<float,3>(layers[5]->get_output_dims(), 150, init, reg_pen));
//	layers[7] = LayerPtr<float,3>(new SigmoidActivationLayer<float,3>(layers[6]->get_output_dims()));
//	layers[8] = LayerPtr<float,3>(new FCLayer<float,3>(layers[7]->get_output_dims(), 10, init, reg_pen));
////	layers[0] = LayerPtr<float,3>(new FCLayer<float,3>(training_prov.get_obs_dims(), 500, init, reg));
////	layers[1] = LayerPtr<float,3>(new SigmoidActivationLayer<float,3>(layers[0]->get_output_dims()));
...
////	layers[4] = LayerPtr<float,3>(new FCLayer<float,3>(layers[3]->get_output_dims(), 10, init, reg));
...
//	MomentumAcceleratedSGDOptimizer<float,3,false> opt(loss, 100);
