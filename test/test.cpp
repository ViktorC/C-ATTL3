...
int main() {
	...
	layers[6] = LayerPtr<float,3>(new FCLayer<float,3>(layers[5]->get_output_dims(), 100, init));
	...
	layers[9] = LayerPtr<float,3>(new TanhActivationLayer<float,3>(layers[8]->get_output_dims()));
	layers[10] = LayerPtr<float,3>(new SoftmaxActivationLayer<float,3>(layers[9]->get_output_dims()));
	...
	NadamOptimizer<float,3,false> opt(loss, reg, 1000);
