...
/**
 * A trait struct for the name of a scalar type and the default numeric
 * constants used for gradient verification depending on the scalar type.
 */
...
struct ScalarTraits<float> {
  ...
  inline static std::string name() { return "float"; }
};
 ...
 */
template <typename Scalar, std::size_t Rank, bool Sequential>
inline std::size_t get_rows(const typename std::enable_if<!Sequential, std::array<std::size_t, Rank>>::type& dims) {
  return dims[0];
}
 ...
 * @return A tensor of the specified dimensions filled with random values in the
 * range of -1 to 1.
 */
...
/**
 * @param dims The dimensions of the random tensor to create.
 * @param non_label The value of the non-label elements of the objective tensor
 * (0 for cross entropy, -1 for hinge loss).
 * @return A one-hot tensor of the specified dimensions in which only one
 * element per sample and time step is 1 while all others equal non_label.
 */
template <typename Scalar, std::size_t Rank, bool Sequential>
inline TensorPtr<Scalar, Rank> random_one_hot_tensor(const std::array<std::size_t, Rank>& dims, Scalar non_label = 0) {
  TensorPtr<Scalar, Rank> tensor_ptr(new Tensor<Scalar, Rank>(dims));
  ...
  for (int i = 0; i < mat_map.rows(); ++i) mat_map(i, dist(rng)) = 1;
  return tensor_ptr;
}
 ...
 * @param non_label The value of the non-label elements of the objective tensor
 * (0 for cross entropy, -1 for hinge loss).
 * @return A multi-label objective tensor whose elements equal either 1 or
 * non_label.
 */
template <typename Scalar, std::size_t Rank>
 ...
 */
template <typename Scalar, std::size_t Rank>
inline KernelPtr<Scalar, Rank> kernel_layer(
    const typename std::enable_if<Rank != 1, Dimensions<std::size_t, Rank>>::type& input_dims) {
  return KernelPtr<Scalar, Rank>(new ConvKernelLayer<Scalar, Rank>(
      input_dims, input_dims.template extend<3 - Rank>()(2), std::make_shared<HeParameterInitialization<Scalar>>()));
}
...
inline KernelPtr<Scalar, Rank> kernel_layer(
  ...
  return KernelPtr<Scalar, Rank>(new DenseKernelLayer<Scalar, Rank>(
      input_dims, input_dims.get_volume(), std::make_shared<GlorotParameterInitialization<Scalar>>()));
}
...
/**
 * @param input_dims The input dimensions of the neural network.
 * @return A simple feed-forward neural network with a sigmoid activation
 * function and hence output values between 0 and 1.
 */
template <typename Scalar, std::size_t Rank>
...
/**
 * @param input_dims The input dimensions of the neural network.
 * @return A simple feed-forward neural network with a tanh activation function
 * and hence output values between -1 and 1.
 */
template <typename Scalar, std::size_t Rank>
...
/**
 * @param input_dims The input dimensions of the neural network.
 * @return A simple feed-forward neural network with a softmax activation
 * function and hence output values between 0 and 1.
 */
template <typename Scalar, std::size_t Rank>
...
/**
 * @param input_dims The input dimensions of the neural network.
 * @return A simple recurrent neural network without an identity output
 * activation function and with a single output time step.
 */
template <typename Scalar, std::size_t Rank>
inline NeuralNetPtr<Scalar, Rank, true> recurrent_neural_net(
    const Dimensions<std::size_t, Rank>& input_dims,
    std::function<std::pair<std::size_t, std::size_t>(std::size_t)> seq_schedule_func) {
  return NeuralNetPtr<Scalar, Rank, true>(new RecurrentNeuralNetwork<Scalar, Rank>(
      kernel_layer<Scalar, Rank>(input_dims), kernel_layer<Scalar, Rank>(input_dims),
      kernel_layer<Scalar, Rank>(input_dims),
...
/**
 * @param input_dims The input dimensions of the neural network.
 * @return A simple single-output recurrent neural network without an identity
 * output activation function and with a single output time step.
 */
template <typename Scalar, std::size_t Rank>
inline NeuralNetPtr<Scalar, Rank, true> single_output_recurrent_neural_net(
    const Dimensions<std::size_t, Rank>& input_dims,
    std::function<std::pair<std::size_t, std::size_t>(std::size_t)> seq_schedule_func) {
  auto init = std::make_shared<GlorotParameterInitialization<Scalar>>();
  return NeuralNetPtr<Scalar, Rank, true>(new RecurrentNeuralNetwork<Scalar, Rank>(
      kernel_layer<Scalar, Rank>(input_dims), kernel_layer<Scalar, Rank>(input_dims),
      KernelPtr<Scalar, Rank>(new DenseKernelLayer<Scalar, Rank>(input_dims, 1, init)),
      ActivationPtr<Scalar, Rank>(new TanhActivationLayer<Scalar, Rank>(input_dims)),
...
inline void print_test_header(std::string test_case_name, std::string test_name) {
  ...
  std::string header = "|   " + test_case_name + ": " + test_name + "; SCALAR TYPE: " + ScalarTraits<Scalar>::name() +
                       "; RANK: " + std::to_string(Rank) + "; SEQ: " + std::to_string(Sequential) + "   |";
  ...
  std::cout << std::endl
            << header_border << std::endl
            << upper_header_padding << std::endl
            << header << std::endl
            << lower_header_padding << std::endl
            << header_border << std::endl;
}
