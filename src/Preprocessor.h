...
//		assert(rows > 0);
//		dims = Utils<Scalar>::template get_dims<Base::DATA_DIMS>(data).template demote<Base::DATA_DIMS - Rank>();
...
//		int rows = data.dimension(0);
//		assert(rows > 0);
//		Dimensions<int,Rank> demoted_dims = Utils<Scalar>::template get_dims<Base::DATA_DIMS>(data).template demote<>();
...
//		int rows = data.dimension(0);
//		Dimensions<int,3> demoted_dims = Utils<Scalar>::template get_dims<4>(data).template demote<>();
//		assert(demoted_dims == dims && "mismatched fit and transform input tensor dimensions");
//		assert(rows > 0);
...
//		if (!reduce_dims)
//			output_dims = Utils<Scalar>::template get_dims<Rank + 1>(data).template demote<>();
