...
Optimizer::Optimizer(Loss* cost, Regularization* reg) :
	cost(cost),
	reg(reg) { };
Optimizer::~Optimizer() {
	delete cost;
...
double Optimizer::calculate_error(NeuralNetwork& net, std::vector<double>& input,
		std::vector<double>& obj) {
	std::vector<double>* output = net.feed_forward(input);
	std::vector<double>& output_ref = *output;
	double error = cost->function(output_ref, obj);
	delete output;
	return error;
};
