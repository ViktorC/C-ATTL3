...
using LossSharedPtr = std::shared_ptr<Loss<Scalar,Rank>>;
...
public:
	Optimizer(LossSharedPtr<Scalar,Rank> loss) :
				loss(loss) {
		assert(loss != nullptr);
	};
	virtual ~Optimizer() = default;
	inline bool verify_gradients(NeuralNetwork<Scalar,Rank>& net, DataProvider<Scalar,Rank>& provider, Scalar step_size = 1e-5,
	...
	inline void optimize(NeuralNetwork<Scalar,Rank>& net, DataProvider<Scalar,Rank>& training_prov,
			DataProvider<Scalar,Rank>& test_prov, unsigned epochs, unsigned early_stop = 0) {
...
protected:
	virtual void fit(NeuralNetwork<Scalar,Rank>& net) = 0;
	virtual Scalar train(NeuralNetwork<Scalar,Rank>& net, DataProvider<Scalar,Rank>& training_prov, unsigned epoch) = 0;
	virtual Scalar test(NeuralNetwork<Scalar,Rank>& net, DataProvider<Scalar,Rank>& test_prov, unsigned epoch) = 0;
...
template<typename Scalar, size_t Rank>
class SGDOptimizer : public Optimizer<Scalar,Rank> {
public:
	SGDOptimizer(LossSharedPtr<Scalar,Rank> loss, RegPenSharedPtr<Scalar> reg, unsigned batch_size) :
				Optimizer<Scalar,Rank>::Optimizer(loss),
				reg(reg),
...
protected:
	inline Scalar train(NeuralNetwork<Scalar,Rank>& net, DataProvider<Scalar,Rank>& training_prov, unsigned epoch) {
		...
		while (training_prov.has_more()) {
			...
			Optimizer<Scalar,Rank>::backpropagate(net, Optimizer<Scalar,Rank>::loss->d_function(out, data_pair.second) /
					instances);
			for (unsigned k = 0; k < layers.size(); k++) {
				Layer<Scalar,Rank>& layer = *(layers[k]);
				if (Optimizer<Scalar,Rank>::is_parametric(layer)) {
					update_params(layer, k, epoch - 1);
					Optimizer<Scalar,Rank>::enforce_constraints(layer);
				}
			}
		}
		return training_loss / instances;
	};
	inline Scalar test(NeuralNetwork<Scalar,Rank>& net, DataProvider<Scalar,Rank>& test_prov, unsigned epoch) {
		// TODO Handle overflow.
		Scalar obj_loss = 0;
		std::vector<Layer<Scalar,Rank>*> layers = Optimizer<Scalar,Rank>::get_layers(net);
		...
		for (unsigned j = 0; j < layers.size(); j++)
			reg_loss += reg->function(Optimizer<Scalar,Rank>::get_params(*(layers[j])));
...
template<typename Scalar, size_t Rank>
class VanillaSGDOptimizer : public SGDOptimizer<Scalar,Rank> {
public:
	VanillaSGDOptimizer(LossSharedPtr<Scalar,Rank> loss, RegPenSharedPtr<Scalar> reg, unsigned batch_size = 1,
			Scalar learning_rate = 1e-3) :
				SGDOptimizer<Scalar,Rank>::SGDOptimizer(loss, reg, batch_size),
				learning_rate(learning_rate) {
		assert(learning_rate > 0);
	};
protected:
	inline void fit(NeuralNetwork<Scalar,Rank>& net) { };
...
template<typename Scalar, size_t Rank>
class MomentumAcceleratedSGDOptimizer : public SGDOptimizer<Scalar,Rank> {
public:
	MomentumAcceleratedSGDOptimizer(LossSharedPtr<Scalar,Rank> loss, RegPenSharedPtr<Scalar> reg,
...
protected:
	inline void fit(NeuralNetwork<Scalar,Rank>& net) {
		...
		for (unsigned i = 0; i < param_grads_vec.size(); i++) {
			...
			param_grads = Matrix<Scalar>(Optimizer<Scalar,Rank>::get_param_grads(layer).rows(),
	...
	inline void update_params(Layer<Scalar,Rank>& layer, unsigned i, unsigned epoch) {
		...
		Matrix<Scalar>& param_grads = param_grads_vec[i];
		Matrix<Scalar>& params = Optimizer<Scalar,Rank>::get_params(layer);
		param_grads = momentum * param_grads - learning_rate * (Optimizer<Scalar,Rank>::get_param_grads(layer) +
				SGDOptimizer<Scalar,Rank>::reg->d_function(params));
		params += param_grads;
	};
...
template<typename Scalar, size_t Rank>
class AdagradOptimizer : public SGDOptimizer<Scalar,Rank> {
public:
	AdagradOptimizer(LossSharedPtr<Scalar,Rank> loss, RegPenSharedPtr<Scalar> reg, unsigned batch_size = 1,
...
template<typename Scalar, size_t Rank>
class RMSPropOptimizer : public AdagradOptimizer<Scalar,Rank> {
public:
	RMSPropOptimizer(LossSharedPtr<Scalar,Rank> loss, RegPenSharedPtr<Scalar> reg, unsigned batch_size = 1,
			Scalar learning_rate = 1e-3, Scalar l2_decay = 1e-1, Scalar epsilon = Utils<Scalar>::EPSILON) :
				AdagradOptimizer<Scalar,Rank>::AdagradOptimizer(loss, reg, batch_size, learning_rate, epsilon),
				l2_decay(l2_decay) {
...
template<typename Scalar, size_t Rank>
class AdadeltaOptimizer : public SGDOptimizer<Scalar,Rank> {
public:
	AdadeltaOptimizer(LossSharedPtr<Scalar,Rank> loss, RegPenSharedPtr<Scalar> reg, unsigned batch_size = 1,
...
template<typename Scalar, size_t Rank>
class AdamOptimizer : public SGDOptimizer<Scalar,Rank> {
public:
	AdamOptimizer(LossSharedPtr<Scalar,Rank> loss, RegPenSharedPtr<Scalar> reg, unsigned batch_size = 1,
